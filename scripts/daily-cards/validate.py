...
def main() -> None:
    ...
    for index, path in enumerate(files, 1):
        ...
        rendered = bool(list((OUTPUT / "full").glob(f"day-{index:02d}-*.png")))
        if index in PROTOTYPES or rendered:
            if index in PROTOTYPES and payload.get("prototypeType") != PROTOTYPES[index]:
                day_errors.append("prototype type mismatch")
            if unresolved:
                day_errors.append(f"rendered day has {unresolved} unresolved coordinates")
    ...
    if args.write_report:
        ...
        rendered_days = [item["day"] for item in day_results if item["rendered"]]
        prototype_lines = []
        for item in day_results:
            if item["prototype"] or item["rendered"]:
...
- Rendered artifacts: Day {", ".join(f"{d:02d}" for d in rendered_days)}
- Errors: {len(errors)}
