...
class TestValidationGuards(unittest.TestCase):
    ...
    def test_missing_required_h2(self):
        # Remove a required H2 from Nice chapter
        bad_content = self.nice_backup.replace("## 꼭 경험할 세 장면", "## 다른 장면")
    ...
    def test_out_of_order_h2(self):
        # Swap order of two required H2s in Nice chapter
        bad_content = self.nice_backup.replace(
    ...
    def test_duplicate_h2(self):
        # Add a duplicate H2 in Nice chapter
        bad_content = self.nice_backup.replace(
    ...
    def test_unknown_content_schema(self):
        # Set schema to unknown-schema
        bad_content = self.nice_backup.replace("content_schema: rs-region-v1", "content_schema: unknown-schema")
        NICE_CHAPTER.write_text(bad_content, encoding="utf-8")
        success, output = self.run_validation()
        self.assertFalse(success)
