...
{pager}"""
    ...
    # 43일 전수. 지역명·특정 Day 예외를 두지 않는다 (R4-05 에서 Day 5·6 예외를 제거했다).
    if fat_missing:
        print(f"피로도 커버리지 가드 실패 — 값 없는 날 {sorted(fat_missing)} (43일 전수 필요)")
        sys.exit(1)
    print(f"  데일리 카드: 43일 → daily/day-01~43.html (Phase4 적용 {len(PHASE4_DAYS)}일)"
          f" · 피로도 {n_fat}일 · 시간표 {n_tt}건 · 커버리지 가드 43일 전수")
...
# 폰트 커버리지에 기대는 기하 도형·딩벳·박스드로잉. CLAUDE.md 가 금지한 부류
# (◉ ▤ ◇ ▧)와 걷어낸 것들(■ ● ▨ ◈ ★ ▾ ▴, 그리고 의사결정 트리를 중첩 목록으로
# 바꾸며 없앤 ─ │ ├ └)을 output 에서 영구 차단한다. 마스크·컴포넌트·목록으로
# 바꿨으니 다시 새면 그건 회귀다. 번들 폰트 unicode-range 밖이라 기기에 따라
# 두부(□)가 된다 — 현장에서 못 쓰는 화면이 되는 것.
#
# 아직 남아 별도 단계에서 처리할 것은 여기 넣지 않는다 — 넣으면 지금 빌드가
# 멈춘다: ⚠(신뢰성 있는 경고 기호라 유지), ○ ✓ ✗ △ 등 의미 표기. 각각 해소될
# 때 이 집합에 더한다.
BANNED_GLYPHS = "◉▤◇▧■●▨◈★▾▴─│├└"
