...
def check_phase6_map_guards():
    """8개 실행지도·80개 공개 기준점·43일 라우팅을 하나의 계약으로 검사한다."""
    problems, total = [], 0
    ...
    if total != 80:
        problems.append(f"GeoJSON 공개 기준점 총 {total}개 (기대 80; 비공개 숙소 제외)")
    for n in range(1, 44):
    ...
    if problems:
        ...
        for p in problems[:30]:
            print("  " + p)
        sys.exit(1)
    print("Phase 6 실행지도 가드: 8지역 · 공개 기준점 80개 · 43일 라우팅 · 전환일 7일 이상 없음")
