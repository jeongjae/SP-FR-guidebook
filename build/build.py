...
# Phase 5 배치 전환 — 배치 1~8 (Day 1~40) + 파일럿 Day 6. Day 4 는 비지도 이동일.
...
def check_phase6_map_guards():
    ...
    if total != 103 or len(google_places) != 103:
        problems.append(f"지도 기준점 총 {total}개 (기대 103) / 레지스트리 {len(google_places)}개 (기대 103)")
    for n in range(1, 44):
    ...
    if problems:
        ...
        for p in problems[:30]:
            print("  " + p)
        sys.exit(1)
    print("Phase 6 실행지도 가드: 8지역 전부 Google Maps · 기준점 103개(레지스트리 기준) · 43일 라우팅 이상 없음")
