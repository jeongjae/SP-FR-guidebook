...
Day 24에 아비뇽에서 렌터카를 반납했다. 이후 19일 — 리옹 4박, 파리 16박 — 은 전부 기차와 도보와 지하철이다. **여행의 성격이 여기서 바뀐다.**
...
기존 원고가 이 구간을 '미식도시'로 잡았다. 그런데 **이 여행에서 미식에 쓸 수 있는 저녁은 3–4회뿐**이다. 선택이 필요하다. 아래 먹거리 참조.

## 시작 전에 확인할 요일 두 개

### Day 24는 월요일이다

크루아루스 시장은 월요일을 제외하고 매일 열린다. 알 폴 보퀴즈는 화요일부터 일요일까지 열고 월요일은 닫는다.

**도착일에는 시장이 없다.** 기존 원고가 Day 24를 '도착·Ainay와 Presqu'île 적응'으로 잡은 것이 맞다.

### Day 27 안시에는 구시가 시장이 없다

구시가 시장은 뤼 생트클레르와 구시가지 거리에서 **화요일·금요일·일요일** 오전 7시부터 13시까지 연중 열린다.

**9월 24일은 목요일이다.** 그 유명한 안시 구시가 시장은 서지 않는다.

기존 원고의 Day 27 '중요한 요일 정보' 항목이 이것으로 보인다. **모르고 가면 실망한다.**

> **대신 얻는 것** — 10시 30분이면 구시가지가 어깨를 부딪칠 정도가 된다는 시장일의 혼잡을 피한다. 운하와 호수를 조용히 볼 수 있다.
...
기존 원고의 **"리옹을 보기 전에 도시를 읽는 곳"** 이라는 정리가 정확하다.

> **먼저 올라가라**
> Day 25의 동선이 **위에서 아래로** 설계돼 있다. 푸르비에르 → 구시가지 → 강. 이것은 관광 편의가 아니라 **도시가 실제로 내려온 순서**다.
...
⚠ 바실리카의 건축 연대·건축가, 푸니쿨라 운행, 박물관 운영은 조사가 부족하다 {{badge:pending|보완 필요}}.
...
> **Parc de la Tête d'Or가 그 다른 곳이다.** Day 26 동선이 이미 그렇게 잡혀 있다.
...
**Day 26(수요일)은 어느 출처로도 정상 운영이다.** 다만 **정확한 마감 시각은 출발 전 공식 확인이 필요하다** {{badge:pending|확인 필수}}.
...
호수가 있는 대공원이다. 기존 원고가 **"장기여행의 회복공간"** 으로 정리한 곳이다.

**43일 중 26일차다.** 이 시점에 공원이 일정에 들어 있는 것은 관광이 아니라 **회복 설계**다.

> **알에서 산 것을 여기서 먹는다.** 좌석 부족 문제가 이렇게 풀린다.

⚠ 면적·개장 시간·러닝 코스 상세는 조사가 부족하다 {{badge:pending|보완 필요}}.

### Day 27 — Annecy 당일치기

### ⚠ 목요일이라 구시가 시장이 없다

위 경고 참조. 구시가 시장은 화·금·일요일 7–13시다.

화요일은 지역 농산물 중심의 식품 시장이고, 금요일과 일요일에는 공산품과 직물까지 더해져 규모가 커진다.

**목요일 대안** — 노벨(Novel) 지구에 목요일 6–13시 시장이 있다. 다만 구시가지가 아니다. 관광 동선에서 벗어난다.
...
기존 원고에 '카드 D — Annecy 크루즈' 예약 항목이 있다. {{badge:pending|운항사·소요·요금 확인 필요}}
...
> 파르디외 역에서 도보 5분 거리다.
> Day 28 출발 전에 시간이 남으면 여기서 파리행 기차 안에서 먹을 것을 산다. **기존 원고의 '생활권의 마지막 아침'과 맞는다.**
...
## 저녁 배치 — 4박에 무엇을 넣을 것인가

| Day | 날짜 | 요일 | 성격 | 권장 |
|---|---|---|---|---|
| 24 | 9/21 | 월 | 도착일 · **시장 휴장** | 숙소 근처 가볍게 |
| 25 | 9/22 | 화 | 도시 탐색일 | **부숑 한 번** |
| 26 | 9/23 | 수 | 시장·공원일 | 알에서 사서 숙소 또는 가볍게 |
| 27 | 9/24 | 목 | 안시 당일치기 | 안시 점심에 무게. 저녁은 가볍게 |
| 28 | 9/25 | 금 | 파리 이동 | — |

**대표 저녁은 Day 25 한 번이 현실적이다.** 기존 원고의 '카드 E — 대표 저녁'이 이것이다. **예약이 필요하다.**
...
시장은 리옹식 피크닉을 꾸리는 가장 좋고 싼 방법이다 — 빵·치즈·소시송·과일로 1인당 €5–12 정도.

토요일 오전이 도시 전역에서 최고의 시장일이고, 8시 30분에서 11시 사이에 가는 것이 좋다.

> **다만 이 일정에 토요일이 없다.** Day 26 수요일이 유일한 시장일이다.
...
Day 24에 반납했다. **이후 전부 대중교통이다.**

## Day 24 — 아비뇽에서 리옹으로 {{badge:p0|P0 연결}}

Avignon 챕터에서 이어진다. 요점만 반복한다.

| 항목 | 내용 |
|---|---|
| 반납 | Avignon TGV |
| ⚠ | **Avignon TGV와 Avignon Centre는 다른 역이다** |
| 완충 | 반납 90분 + 승차 30분 |
| 도착 | Lyon Part-Dieu 또는 Perrache {{badge:pending|확인}} |
...
> **푸니쿨라가 TCL 요금제에 포함되는지 확인하라** {{badge:pending|확인 필요}}. Day 25에 푸르비에르를 오를 때 쓴다.
...
**Day 28 파리행은 파르디외 출발이 기본이다.** 표의 출발역을 확인할 것.

## Day 27 — 안시 왕복

기존 원고에 '카드 C — Annecy 왕복' 예약 항목이 있다.
...
## Day 28 — 파리로 {{badge:p0|P0 연결}}

| 항목 | 내용 |
|---|---|
| 출발 | Lyon Part-Dieu |
| 도착 | Paris Gare de Lyon {{badge:pending|확인}} |
| 소요 | 2시간 내외 {{badge:pending|확인}} |
| 완충 | 60분 이상 |
| 짐 | **43일 중 28일차.** 짐이 늘어 있다 |

## 안전

기존 원고가 '4. 치안 판단과 여행 설계 반영'을 별도 장으로 두고 **"프랑스 범죄율 1위"라는 주장에 대한 판단**을 다룬다. 그 판단을 유지하되 실무 원칙만 반복한다.
...
| 항목 | 방법 | 효과 |
|---|---|---|
| **TGV 사전 예매** | Day 28 파리행 | **가장 크다** |
| 안시 왕복 | 사전 예매 | 큼 |
| 점심 | 시장 조달 (Day 26) | 1인당 €5–12 |
...
| 순위 | 대상 | 이유 |
|---:|---|---|
| 1 | **Day 28 파리행 TGV** | 가격 변동이 크다 |
| 2 | **Day 27 안시 왕복** | 좌석·시각 |
| 3 | **Day 25 대표 저녁** | 부숑은 작다. 예약 필수 |
| 4 | 안시 크루즈 | 운항 시각 |
| 5 | 리옹 숙소 | — |
...
| 순위 | 대안 | 빼야 할 것 | 추가 | 난이도 |
|---:|---|---|---|---|
| 1 | **Musée des Confluences** | Day 26 공원 | 2h | 낮음 |
| 2 | **Musée des Beaux-Arts** | Day 25 오후 | 2h | 낮음 |
| 3 | **Musée des Tissus** | Day 26 크루아루스 대체 | 1.5h | **비엔날레 예외 개관 기간 안** |
| 4 | **Pérouges** | Day 27 안시 대체 | 반나절 | 중간 |
| 5 | Vienne | Day 27 대체 | 반나절 | 중간 |
| 6 | Beaujolais 와인 | 하루 | 하루 | 높음 (운전) |

## Musée des Confluences {{badge:rest|1순위}}

두 강이 만나는 지점의 현대 박물관이다. 기존 원고의 '13.5 Part-Dieu·Confluence — 현대도시의 두 방향'에서 언급된 지역이다.

**왜 1순위인가** — 이 구간이 **로마·르네상스·19세기 산업**으로 구성돼 있는데 **현대가 빠져 있다.** 콩플뤼앙스가 그 자리를 채운다. 건물 자체가 볼거리로 알려져 있다.

**넣는 법** — Day 26 오후. 공원 대신. 다만 **회복일의 성격을 잃는다.**
...
**왜 뺐나** — 이 여행에는 파리 16박이 남아 있다. **미술관은 파리에서 충분하다.**
**넣는 법** — 우천 시 Day 25 오후.
...
**왜 언급하는가** — Day 26에 크루아루스에서 **노동의 공간**을 보는데, 여기는 **생산물**을 본다. 짝이 맞는다.

✓ **2018년부터 폐관 중이고, 전면 재개관은 2029년 목표다.** 그런데 **2026년 리옹 비엔날레 기간(9/19–12/13)에 예외적으로 문을 연다.** 9년 만이다. **리옹 체류 9/21–9/25 가 이 기간 안에 든다** — 이 여행에서만 가능한 조건이다.
...
**왜 뺐나** — Day 22에 퐁 뒤 가르, Day 23에 (선택 시) 글라눔을 봤다. **로마가 충분하다.**

## Beaujolais

리옹 북쪽 와인 산지다.

**왜 뺐나** — **차가 없다.** Day 24에 반납했다. 투어를 쓰면 가능하지만 하루가 든다.
...
| 항목 | 이유 |
|---|---|
| **Day 24 오후 관광** | 아비뇽 반납 + TGV 이동일 |
| **Day 28 오전 관광** | 파리행. 짐이 있다 |
...
| 항목 | 상태 |
|---|---|
| **Annecy 구시가 시장 화·금·일 7–13시** | **복수 출처 확인** — 9/24 목요일은 해당 없음 |
...
| **Palais de l'Île 운영·휴관일·요금** | **복수 출처 확인 — 화요일 휴관 맞다.** 여름(6/1–9/30) 10:30–18:00 · 겨울(10/1–5/31) 10:00–12:30, 14:00–17:30. **Day 27(9/24)은 여름 시간이다.** 일반 €4(7·8월 €5) · 감액 €2 · 12세 미만 무료. 마지막 입장 폐관 45분 전. 출처: musees.annecy.fr 공식 · 오트사부아 관광청 {{badge:pending|2026-08 확인 · 출발 전 재확인}} |
| **Annecy 크루즈 운항** | **부분 확인 · 주의** — Compagnie des Bateaux du lac d'Annecy 가 1시간·1시간 30분 해설 크루즈를 운항한다. **8/29–9/20 은 화·수·금·일 하루 2편의 비수기 시간표**다. ⚠ **Day 27(9/24)은 목요일이고 그 창(9/20) 이후다 — 이 시간표에 목요일이 없다.** 9/21 이후 시간표는 공개 정보로 확인되지 않았다. **안시 당일치기에서 크루즈를 확정 일정으로 넣지 말 것.** 출발 전 bateaux-annecy.com 에서 9/24 운항 여부를 직접 확인해야 한다 {{badge:pending|2026-08 확인 · 출발 전 재확인}} |
...
| **TCL 요금 체계·푸니쿨라 포함 여부** | **복수 출처 확인 — 푸니쿨라는 포함된다.** 단발권은 **1시간 유효, 메트로·트램·버스·푸니쿨라 전부에 환승 무제한**. 존 1–2 €2.10 · 전 존 €3.70. 푸니쿨라만 왕복 이용하면 €3.60. 성인 월정기 €75.90(존 1–2). **2026/9/1 부터 존 체계가 붙은 새 요금표가 적용된다 — 체류가 9/21–9/25 라 새 요금표 구간이다.** 출처: tcl.fr 공식 · 리옹 메트로폴 {{badge:pending|2026-08 확인 · 출발 전 재확인}} |
...
| **Part-Dieu / Perrache 출발역** | **복수 출처 확인** — 파리행 TGV 는 **두 역 모두**에서 나가고 종점은 **Paris Gare de Lyon** 이다. 다만 **Part-Dieu 약 1시간 54분, Perrache 2시간 10–15분**으로 차이가 난다. **Day 28(파리 이동)은 Part-Dieu 출발편이 맞다** — 원고의 판단과 같다 {{badge:pending|2026-08 확인 · 출발 전 재확인}} |
| **Musée des Tissus 현행 운영** | **복수 출처 확인** — 2018년부터 폐관, 전면 재개관은 2029년 목표. **단 2026 리옹 비엔날레(9/19–12/13) 기간에 예외 개관한다.** 리옹 체류 9/21–9/25 가 이 기간 안에 든다. 출처: 오베르뉴론알프 지역 · Lyon Capitale · museedestissus.fr. 개관 요일·시각·요금은 {{badge:pending|2026-08 확인 · 출발 전 재확인}} |
