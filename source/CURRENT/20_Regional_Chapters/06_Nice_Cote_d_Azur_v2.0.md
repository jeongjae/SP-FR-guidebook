...
니스 체류는 43일간의 장기 유럽 여정 중 가장 긴 무이동 구간(5박 6일)이다. 짐을 매일 싸고 푸는 피로에서 완전히 벗어나 회복을 도모하는 핵심적인 회복 구간이자, 기차로 20~30분 거리인 칸과 모나코를 탐방하며 '생활도시(Nice) - 영화제 리조트(Cannes) - 소국(Monaco)'이라는 지중해의 세 가지 매력을 횡단하여 비교하는 풍성한 인트로 역할을 한다.
...
- **확정 숙소**: **Palais ALZIRA · 12 Rue Verdi, 06000 Nice** (체크인 15:00 · 체크아웃 11:00) · 정원 딸린 로프트 50㎡ · 총액 **€809.54 결제 완료** (Airbnb HMJ3HX8QAY) · 호스트 Catherine **+33 6 21 70 18 70**.
  Masséna 광장 북서쪽의 Verdi 가에 위치하여, 도보로 역(Nice-Ville) 및 해변(Promenade) 모두 10~15분 이내 접근이 가능하다. 주방 시설과 세탁기를 갖춘 50㎡ 로프트 형태로 5박의 생활형 완충 일정에 완벽하게 맞는다.

## 도착·출발·지역 내 교통

- **공항 도착**: 니스 코트다쥐르 공항에서 시내 진입 시 **트램 2호선(Ligne 2)**을 이용해 숙소 근처 정거장(Alsace-Lorraine)에서 내린다. 단발권은 €1.80이며, 도시 이동이 잦은 날은 1일권(€5.00)이 경제적이다.
- **철도 당일치기**: 칸과 모나코는 니스-빌(Nice-Ville) 역에서 **TER 열차**를 이용한다. 각각 35분, 25분 소요되며 예약을 강제하지 않고 모바일 앱이나 자판기에서 즉석 발권 가능하다. 모나코 시내 이동 시 오르막이 부담되면 시내버스 CAM 1·2번(편도 **€2.00**, 1일권 Pass 24h **€5.50**)을 활용한다.
- **렌터카 인수**: **Day 12(9/9) 오전 09:00**, Nice역 Hertz 영업소(Avenue Thiers)에서 자동변속 컴팩트 차량을 인수하여 생폴드방스를 거쳐 프로방스로 이동한다.

## 핵심 셀프가이드

이 구역의 셀프 가이드는 아래 11개 세부 항목을 정본으로 삼으며, 빌드 스크립트를 통해 개별 페이지로 연결된다.
...
- **놓치지 말 것**: 화덕에서 갓 구워낸 소카(Socca)를 즉석에서 손으로 찢어 먹는 맛을 놓치지 말 것. 월요일은 꽃시장 대신 골동품 시장(Brocante)이 열리므로 일요일 오전(Day 8) 방문이 알맞다.
...
칸(Cannes) 시청 뒤편에 위치한 철골 지붕 구조의 유서 깊은 재래시장이다.
...
*   **도보 경로 및 관찰 지점**:
    ...
    2. **Place Rossetti**: 성당 내부를 조용히 관람하고 광장 앞의 전통 젤라토를 가볍게 맛본다.
...
- **니스 요리의 문법**: "이탈리아 식재료를 프랑스 바로크적 문법으로 해석한다." 500년간 이어진 사보이 왕가의 영향으로 파스타, 올리브 오일, 바로크 양식의 식문화가 단단하게 조화되어 있다.
- **반드시 먹을 세 가지**:
...
- **식당 전략**:
  ...
  - **La Table Alziari**: 올리브유와 제철 생선 요리가 깔끔하여 속 편한 저녁을 원할 때 알맞다.

## 당일치기·우천·피로 대안

- **근교 대체안**:
  - **Villefranche-sur-Mer**: 니스 바로 옆 만. Nice-Ville ↔ Villefranche-sur-Mer 기차 TER 편도 **€2.30** (약 7분). 회복일의 성격을 해치지 않는 가장 가벼운 바다 산책 추가안이다.
  - **Èze Village**: 절벽 위 중세 마을. Nice Vauban ↔ Èze Village Lignes d'Azur 82번 버스 편도 **€1.70** (약 30분).
- **우천 대안**:
...
- **Cannes Forville 시장**: 월요일은 일반 식재료 시장 대신 골동품 시장(Brocante)으로 운영되므로, 일요일(Day 9) 아침 방문을 유지한다.
