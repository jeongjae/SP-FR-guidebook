---
title: "Jason과 Julia의 유럽 장기여행 가이드 — Gordes와 Luberon 마을"
chapter: 08
version: "2.0"
content_schema: rc-region-v1
status: "Gordes 2박 실행일정·마을 콘텐츠 보강본 — 예약 전 최종 확인 필요"
travel_dates: "2026-09-13 – 2026-09-15"
travelers: "Jason · Julia"
last_web_verification: "2026-08-01"
source_priority: "공식 관광기구·지자체·시설·숙박업체 → 지역 관광 데이터베이스 → 예약 플랫폼 보조 확인"
---

![Luberon 대표사진](../../ASSETS/88_Representative_Public_Photos/05_Luberon_Gordes_CC_BY_SA_4_0.jpg)

*Gordes — Photo: Einaz80, CC BY-SA 4.0; [원문](https://commons.wikimedia.org/wiki/File:Village_of_Gordes.jpg). 가이드북용으로 크롭·리사이즈.*

# Commercial Guide Module

> **Gordes 2박을 거점으로 오크르·석조마을·Sénanque를 잇는 Luberon 구간**
> Optional을 모두 빼도 각 Day의 핵심 경험과 식사·체크인이 남는다.

## Editor’s Verdict — 이 지역에 시간을 쓸 가치와 한계

Luberon에서는 Gordes에 2박하며 서로 다른 풍경과 돌빛을 가진 마을을 연결한다. Day 16의 Lourmarin과 Gordes, Day 17의 Roussillon과 Sénanque가 이 구간의 정체성을 만든다.

Bonnieux·Lacoste·Ménerbes·Goult는 시간이 남을 때만 붙인다. Optional을 위해 Lourmarin 점심, Gordes 체크인, Roussillon 오커길이나 Sénanque 관람 시간을 희생하지 않는다.

### 이번 2박의 핵심

- **Gordes 2박** — 도착일 늦은 오후와 다음 날 저녁에 관광객이 줄어든 마을을 경험한다.
- **Roussillon의 오커** — 다른 Luberon 마을과 전혀 다른 색과 지질을 직접 걷는다.
- **Gordes와 석조마을** — 돌과 지형이 마을의 형태를 어떻게 바꾸는지 비교한다.

9월 중순은 라벤더보다 포도와 무화과, 수확기의 농촌 풍경을 보는 시기다. 유명마을을 모두 돌기보다 숙소로 돌아오는 시간을 충분히 남긴다.

---

## 한눈에 보기 — 일정

| 날짜 | 핵심 일정 |
|---|---|
| 9/13 일 | Aix 체크아웃 · Lourmarin·점심 · [Bonnieux · Lacoste] · Gordes 체크인·마을 |
| 9/14 월 | Roussillon·Sentier des Ocres · [Ménerbes · Goult] · Sénanque · Gordes |
| 9/15 화 | Gordes 체크아웃 · L'Isle-sur-la-Sorgue · [Fontaine-de-Vaucluse] · Avignon 체크인 |

상세 시각, 주차, 식사와 선택 일정은 각 날짜의 Day 페이지에서 확인한다.

## 석조마을, 오커 지형, 수도원과 물의 Provence

- **대표 촬영 장면:** Gordes 진입로 전경, Roussillon 오커 절벽, Sénanque의 계곡과 로마네스크 건축
- **기념품 방향:** Luberon 와인, 올리브유, chèvre, tapenade와 로컬 꿀
- **일정 삭제 원칙:** Day 16은 Lacoste→Bonnieux, Day 17은 Goult→Ménerbes, Day 18은 Fontaine 순으로 삭제한다.
- **등급 표시:** 필수·우선 추천·선택은 객관적 품질평가가 아니라 이 여행계획에서의 편집 추천도다.

# Regional Context & Scheduled Place Dossiers

## 구역별 이해와 숙소 생활권

Gordes 마을 또는 가까운 생활권의 2박 숙소를 거점으로 삼는다. 이동은 렌터카가 기본이고 각 마을은 외곽 주차 후 걷는다.

첫날 Lourmarin에서 점심을 먹고 Gordes 체크인 후 마을과 저녁을 즐긴다. Day 17은 Sénanque 관람 뒤 Gordes로 돌아와 쉬며, Day 18은 L'Isle를 거쳐 Avignon으로 이동한다.

숙소에 도착하면 주차 위치와 야간 출입방법을 먼저 확인한다.

---

## 도착·출발·지역 내 교통

## 이동은 렌터카

Gordes와 여러 마을을 연결하는 일정은 렌터카가 기본이다. 대중교통으로 같은 다중 마을 동선을 재현하지 않는다.

## 마을에서는 외곽에 주차하고 걷기

석조마을 내부는 길이 좁고 보행자 중심이므로 골목 안으로 차량을 진입시키지 않는다. 마을 입구의 공식 외곽 주차장에 차를 세우고 도보로 진입한다.

## Gordes로 돌아오는 시간

Day 17은 Sénanque 관람 뒤 18:00 전후 Gordes로 돌아와 휴식한다. Optional 때문에 수도원 슬롯이나 저녁 복귀를 늦추지 않는다.

## Avignon으로 출발하는 날

Day 18 체크아웃 후 L'Isle-sur-la-Sorgue의 물길을 걷고 Avignon으로 이동한다. Fontaine-de-Vaucluse는 15:00~16:00 체크인을 해치지 않을 때만 추가한다.

---

## 핵심 셀프가이드

---

#### Lourmarin {{grade:essential|필수}}

> **Editor's Verdict**: 알베르 카뮈가 영면한 뤼베롱 남부의 평탄하고 세련된 예술 마을. 프로방스 최초의 르네상스 성과 플라타너스 카페 테라스, 로즈마리에 덮인 소박한 카뮈의 묘소.

- **체류/요금**: 60–90분 · 마을/묘지 **무료**, 르네상스 성 €7.50
- **상세 가이드**: [Lourmarin 전체 심화 가이드 보기](../places/lourmarin.html)

---

#### Roussillon · Sentier des Ocres {{grade:essential|필수}}

> **Editor's Verdict**: 17가지 타오르는 황토빛(Ochre) 절벽과 소나무 숲이 어우러진 뤼베롱에서 가장 이색적인 붉은 마을. 1억 년 지질학의 기적 '오커 트레일'과 파스텔톤 골목.

- **체류/요금**: 1.5–2시간 · 트레일 €3.50 (어두운 신발 권장)
- **상세 가이드**: [Roussillon · Sentier des Ocres 전체 심화 가이드 보기](../places/roussillon-sentier-des-ocres.html)

---

#### Goult {{grade:optional|선택}}

> **Editor's Verdict (선택 2순위)**: 정상의 Moulin de Jérusalem과 조용한 생활 골목을 45~60분 걷는 마을. Day 17에서 가장 먼저 생략한다.

- **체류/요금**: 45–60분 · **무료** (마을 입구 무료 주차장)
- **상세 가이드**: [Goult 전체 가이드 보기](../places/goult.html)

---

#### Gordes {{grade:essential|필수}}

> **Editor's Verdict**: 보클뤼즈 고원 가장자리 백색 석회암 절벽 위에 층층이 솟아오른 뤼베롱 최고의 요새 마을. 진입로 전망대(Town View Point)의 압도적 파노라마와 11세기 중세 르네상스 성채.

- **체류/요금**: 60–90분 · **무료** (화요일 시장 시 08:30 이전 도착 권장)
- **상세 가이드**: [Gordes 전체 심화 가이드 보기](../places/gordes.html)

---

#### Abbaye Notre-Dame de Sénanque {{grade:essential|필수}}

> **Editor's Verdict**: 1148년 깊은 협곡 속에 세워진 12세기 시토회 로마네스크 수도원. Day 17 월요일 오후 관람을 핵심으로 유지한다.

- **CONFIRMED**: 2026-09-14(월) 비가이드 관람 13:00–17:00 마지막 입장·한국어 HistoPad 제공. 2026-08-25 공식 예매 화면에 15:00·16:00·17:00 회차가 표시됐다.
- **ACTION REQUIRED**: 기존 16:00–17:30 창에 맞는 16:00 HistoPad 회차를 예약하고 티켓을 오프라인 저장한다.
- **FALLBACK**: 예약을 못 하면 계곡 전망·수도원 외관은 MUST로 유지하고 내부만 생략한다. Roussillon 체류를 줄이지 않고 Goult→Ménerbes 순으로 뺈다.
- **체류/요금**: 외관 45–60분 · HistoPad 내부 약 1–1.5시간
- **상세 가이드**: [Abbaye de Sénanque 전체 가이드 보기](../places/abbaye-de-senanque.html)

---

#### Ménerbes {{grade:optional|선택}}

> **Editor's Verdict (선택 일정)**: 피터 메일의 소설 무대이자 능선 위의 성채 마을. 뤼베롱 와인과 트러플의 중심지 (Day 18 선택 옵션).

- **체류/요금**: 45–60분 · **무료**
- **상세 가이드**: [Ménerbes 전체 가이드 보기](../places/menerbes.html)

---

#### Bonnieux {{grade:optional|선택}}

> **Editor's Verdict (Day 16 선택)**: 뤼베롱 산맥 북사면에 솟아오른 계단식 언덕 마을. Lacoste보다 우선하지만 Gordes 체크인이 늦어지면 생략한다.

- **체류/요금**: 60–75분 · **무료**
- **상세 가이드**: [Bonnieux 전체 가이드 보기](../places/bonnieux.html)

---

#### Lacoste {{grade:optional|선택}}

> **Editor's Verdict (Day 16 선택)**: Bonnieux 맞은편 언덕의 석조마을. Day 16에서 가장 먼저 생략한다.

- **체류/요금**: 45–60분 · 마을 무료
- **상세 가이드**: [Lacoste 전체 가이드 보기](../places/lacoste.html)

---

#### L'Isle-sur-la-Sorgue {{grade:essential|필수}}

> **Editor's Verdict**: Day 18 화요일에는 골동품보다 Sorgue 수로·물레방아·강변·구시가지가 우선인 물의 마을이다.

- **체류/요금**: 1.5–2시간 · 마을 무료
- **상세 가이드**: [L'Isle-sur-la-Sorgue 전체 가이드 보기](../places/l-isle-sur-la-sorgue.html)

---

#### Fontaine-de-Vaucluse {{grade:optional|선택}}

> **Editor's Verdict (Day 18 선택)**: Sorgue 수원지와 석회암 절벽을 걷는 60~90분 자연 산책. Avignon 체크인을 해치지 않을 때만 추가한다.

- **체류/요금**: 60–90분 · 마을 무료
- **상세 가이드**: [Fontaine-de-Vaucluse 전체 가이드 보기](../places/fontaine-de-vaucluse.html)

---

## 음식·시장·카페·생활체험

## Luberon에서 맛볼 프로방스 식재료

프로방스 내륙의 식문화는 생산자 직거래 시장과 신선한 제철 식재료를 중심으로 한다.

- **Banon AOP** — 밤나무 잎으로 싸서 숙성시킨 프로방스 전통 염소 젖 치즈.
- **Cavaillon 멜론** — 뤼베롱 인근 카바용의 당도 높은 주황색 과육 멜론.
- **Figues de Solliès** — 9월 제철을 맞은 달콤하고 부드러운 프로방스 무화과.
- **Tapenade & Anchoïade** — 올리브와 멸치로 만든 프로방스식 스프레드.
- **Miel de Provence** — 라벤더와 야생화에서 채취한 프로방스 꿀.
- **Huile d'olive du Luberon** — 과일 향과 쌉싸름한 풍미의 로컬 엑스트라 버진 올리브유.

프로방스에서는 지역 로제 와인도 대표적인 식문화 요소다. 운전하는 날에는 마시지 않고 저녁 식사 때 선택한다.

## 이번 일정의 식사 배치

- **Day 16 · CONFIRMED** — La Récréation(15 rue Philippe de Girard)은 2026 연중 등록, 성수기 7일 운영, 점심 12:00–14:00이다. à la carte €22–26, 성인 메뉴 €37.50–38.50, plat du jour €22.
- **Day 16 · ACTION REQUIRED / FALLBACK** — 9/13 12:15 실제 좌석은 미확인. 예약이 안 되면 같은 12:15–13:30 창에 Lourmarin 마을 카페·브라시리를 현장 이용하고 동선은 바꾸지 않는다.
- **Day 17 · CONFIRMED** — Bistrot Le 5 공식 사이트는 **월요일 휴무**로 표시한다. Day 17 점심으로 의존하지 않고 Ménerbes를 MUST로 승격하지 않는다.
- **Day 17 · FALLBACK** — A) Ménerbes 전 Roussillon 점심, B) Ménerbes 카페·불랑제리 간단식, C) 우회 없이 Gordes 방향 복귀.
- **Day 18** — L'Isle에서는 커피·브런치, Fontaine을 가면 강변 가벼운 점심, 저녁은 Avignon 숙소 주변에 둔다.

Gordes 화요시장은 Day 18 체크아웃과 충돌하므로 참고 정보로만 두고 핵심 일정으로 승격하지 않는다.

---

## 당일치기·우천·피로 대안

## Day 16 Gordes 이동일 우천/지연

- Lourmarin은 마을+점심으로 유지하되 Château 내부관람을 생략한다.
- Lacoste를 먼저, 그래도 늦으면 Bonnieux를 생략하고 Gordes 체크인을 지킨다.

## Day 17 Roussillon 우천/기상악화

- 강풍이나 우천으로 Sentier des Ocres 탐방로가 통제될 경우 마을 내부 산책으로 축소하고 숙소 휴식으로 전환한다.
- Goult를 먼저 삭제하고, 그래도 늦으면 Ménerbes를 축소하거나 생략한다. Sénanque 예약·운영시각을 우선한다.

## Day 18 이동일 우천/피로

- Gordes 시장은 핵심 일정으로 넣지 않고 체크아웃 뒤 L'Isle로 이동한다.
- 비·피로·체크아웃 지연 시 Fontaine-de-Vaucluse를 생략하고 Avignon 체크인을 우선한다.

## 일정 조정 기준

- Optional을 모두 생략해도 Day가 완전하게 성립해야 한다. 식사·휴식·체크인과 MUST 체류시간을 먼저 지킨다.

---

## 예약·비용·안전·주차·귀가

### Gordes 숙소와 생활권

- **ACTION REQUIRED**: Gordes accommodation for 9/13–9/15 is not yet confirmed in the repository.
- **숙소 형태**: Gordes 마을 또는 가까운 생활권의 2박 숙소(미확정). 후보명·Gordes 중심 좌표를 확정 숙소 주소로 사용하지 않는다.
- **일정**: 2026-09-13(일) 체크인 – 2026-09-15(화) 체크아웃 (2박)
- **확정 후 동기화**: 숙소명·주소·체크인 가능시간·주차·연락처·Gordes 중심 이동·Day 16 도착·Day 17 아침 출발 동선을 함께 갱신한다.

### 핵심 안전 및 이동 확인

- **차량 안전**: 이동일(Day 16, Day 18) 수하물이 실린 차량은 반드시 트렁크 가림막을 닫고 귀중품은 휴대한다.
- **마을 주차**: 골목 내 진입을 금지하고 마을 외곽 공식 주차장을 이용한다.
- **오커 탐방로**: 황토 먼지가 묻을 수 있으므로 오염에 강한 어두운 색 신발과 복장을 착용한다.

---

## 공식 확인 정보와 재확인 대상

### 2026년 체류기간 주요 행사

- **Lourmarin Salon du Carnet de Voyage**: 여행스케치 행사 (2026-09-12~13, Day 16)
- **포도 수확기**: 9월 중순 와이너리 수확 및 농촌 풍경

### 출발 전 최종 확인

- [ ] Gordes 숙소 2박 예약 상태 및 체크인 시간 확인
- [ ] Lourmarin La Récréation 9/13 점심 예약 가능 여부 확인
- [ ] Roussillon Sentier des Ocres 개방 상태 확인
- [ ] Sénanque 9/14 16:00 HistoPad 회차 예약·티켓 오프라인 저장
- [ ] 9/15 Avignon 체크인·관리주차장 확인

### 공식 정보원

1. Destination Luberon — https://www.destinationluberon.com/
2. Pays d’Apt Luberon Tourism — https://www.luberon-apt.fr/
3. Ville de Gordes — https://www.gordes-village.com/
4. Roussillon en Provence — https://roussillon-en-provence.fr/
5. Abbaye de Sénanque — https://www.senanque.fr/

---

## 검증 상태 — 보강본 근거

| 항목 | 상태 |
|---|--- |
| Sentier des Ocres (약 30–60분·9월 매일 운영·기상 조건부) | 공식 확인 |
| Sénanque (월요일 관람·HistoPad 한국어·9/14 15:00·16:00·17:00 예매 화면 표시) | 공식 확인 (2026-08-25) |
| Bistrot Le 5 (9/14 월요일 휴무) | 공식 사이트 확인 (2026-08-25) |
| L'Isle 전통시장 (목·일 오전, 화요일은 시장일 아님) | 공식 확인 |
| 주요 antique village의 화·수 휴무 패턴 | 공식 확인 |
| Lourmarin 카뮈 묘소 및 르네상스 샤토 외관 | 복수 출처 확인 |
| Abbaye de Sénanque (12세기 시토회 로마네스크 수도원·9월 라벤더 수확 완료) | 복수 출처 확인 |
| 뤼베롱 내부 D-도로 특성 및 외곽 주차 원칙 | 확인 |
| **운영시간·요금 전반** | **[출발 전 재확인]** — 계절 및 현장 사정에 따른 변동 가능 |

## 실행지도 · 현장 사용

- [대화형 HTML 지도](../../ASSETS/75_Execution_Maps/Luberon_Execution_Map_v0.2.html)
- [GeoJSON](../../ASSETS/75_Execution_Maps/Luberon_Execution_Map_v0.2.geojson)
- [KML](../../ASSETS/75_Execution_Maps/Luberon_Execution_Map_v0.2.kml)
- 대상 일정: **Day 16–18**
- 숙소 핀은 **Gordes 2박 생활권** 기준이다.
- 연결선은 일정상의 기준점 순서를 보여주는 개략선이다. 실제 도보·운전은 Google Maps에서 다시 계산한다.
- HTML 배경지도와 Google Maps 링크는 인터넷 연결이 필요하다.

## Day 16 — 9월 13일 일요일
### Aix 체크아웃 → Lourmarin → Gordes

* **MUST**: Lourmarin, Gordes 체크인·마을
* **OPTIONAL**: Bonnieux, Lacoste
* **IF LATE**: Lacoste → Bonnieux 순으로 삭제한다.

Base는 Aix → Lourmarin → Gordes, Extended는 Bonnieux와 Lacoste를 사이에 넣는다. 상세 시각·주차·식사는 Day 16 정본을 따른다.

---

## Day 17 — 9월 14일 월요일
### Roussillon → 선택마을 → Sénanque → Gordes

* **MUST**: Roussillon·Sentier des Ocres, Sénanque, Gordes 복귀
* **OPTIONAL**: Ménerbes 1순위, Goult 2순위
* **IF LATE**: Goult → Ménerbes 순으로 삭제한다.

Sénanque 월요일 오후 관람 슬롯을 우선한다. 상세 시각·식사·예약 확인은 Day 17 정본을 따른다.

16:00 HistoPad 회차는 2026-08-25 공식 예매 화면에 표시됐으나 아직 예약 완료 상태가 아니다. 예약을 못 하면 외관·계곡을 MUST, 내부를 OPTIONAL을 실행한다. 개인용 사진은 삼각대 없이 가능하지만 예식 중 사진·영상·녹음은 금지다. 9월에 라벤더밭을 주경험으로 표현하지 않는다.

---

## Day 18 — 9월 15일 화요일
### Gordes 체크아웃 → L'Isle-sur-la-Sorgue → Avignon

* **MUST**: L'Isle의 수로·물레방아·구시가지, Avignon 체크인
* **OPTIONAL**: Fontaine-de-Vaucluse
* **IF LATE**: Fontaine을 생략하고 15:00~16:00 Avignon 체크인을 지킨다.

화요일은 L'Isle 전통시장일이 아니고 주요 앤틱단지의 휴무가 많다. 골동품 쇼핑일로 설명하지 않는다.
