---
title: "Jason과 Julia의 유럽 장기여행 가이드 — 리옹과 안시"
chapter: 10
version: "1.2"
status: "상용 가이드북형 장소·식당 보강본 — 치안 통계방법론·Annecy 교통·시장 공식자료 반영"
travel_dates: "2026-09-21 ~ 2026-09-25"
travelers: "Jason · Julia"
last_web_verification: "2026-07-30"
source_priority: "프랑스 내무부·지자체·공식 관광기관·SNCF·TCL·시설 공식자료"
---

# 10. 리옹과 안시 4박 5일
## 두 강의 도시, 르네상스 골목, 비단언덕과 알프스 호수

> **여행의 역할**: 프로방스의 자동차 여행을 마치고 철도·도보 중심의 도시생활로 전환하는 구간이다. 리옹에서는 박물관을 많이 보는 대신 **도시 지형, 르네상스 구시가지, 프레스킬의 광장, 크루아루스의 비단노동자 동네, 시장과 부숑 음식**을 경험한다. 하루는 안시로 당일치기하여 알프스 호수도시라는 완전히 다른 지역성을 추가한다.

> **여행자 기준**: Jason·Julia는 새로운 도시와 지역을 방문하는 경험을 미술관·박물관보다 우선한다. 따라서 Musée des Beaux-Arts, Musée des Confluences, Institut Lumière 등은 기본 일정이 아니라 비·피로·관심도에 따른 선택 모듈로 둔다. Julia의 수영도 선택 일정이며 숙소 선정조건이 아니다.

> **치안에 대한 결론**: 인터넷에서 보이는 “리옹은 프랑스 범죄율 1위 도시”라는 표현은 **프랑스 정부가 발표한 공식 순위가 아니다.** 프랑스 내무부 SSMSI의 2025년 시·구 단위 자료는 14개 주요 범죄·위반 지표를 각각 공개하며, 리옹·파리·마르세유는 구별 자료도 제공한다. 정부는 하나의 ‘도시 위험 총점’을 발표하지 않으므로, 민간사이트가 서로 다른 범주의 건수를 합산한 순위는 포함 범주·인구기준·관광객·통근인구 처리방식에 따라 결과가 달라진다. 리옹 관광청은 리옹권을 안전한 여행지로 안내하면서도 Vieux Lyon과 혼잡한 대중교통의 소매치기를 특히 경고한다. 이번 일정은 강력범죄 공포보다 **관광지 소매치기, Part-Dieu 수하물 관리, 야간 귀가와 숙소 입지**를 현실적으로 관리하도록 설계한다.

> **디자인 메모**: 최종본에는 지도·사진을 넣는다. 현재 작업본에는 삽입 위치와 이미지 제목·설명만 배치한다.

---

# Layer 1. Quick Reference · 확정 일정 · 실행표

## 1. 한눈에 보는 실행 요약

| 항목 | 확정·권장 내용 |
|---|---|
| 체류 | 2026년 9월 21일 월요일 체크인 → 9월 25일 금요일 체크아웃, 4박 |
| 이동 | 9/21 Avignon TGV역 렌터카 반납 → Lyon Part-Dieu TGV. 9/25 Lyon→Paris TGV |
| 여행 주제 | Fourvière 지형 · Vieux Lyon과 traboules · Presqu’île 광장과 강변 · Croix-Rousse 비단동네 · Lyon 음식 · Annecy 호수도시 |
| 기본 숙소권 | **Ainay–Ampère–Bellecour 남부** 1순위. **Brotteaux–Masséna–Foch** 2순위 |
| 숙박 형태 | 4박이므로 아파트호텔 또는 조용한 호텔. 24시간 또는 야간 프런트, 엘리베이터, 냉방, 객실금고, 밝은 귀가동선 확인 |
| 계획 숙박예산 | 2인 4박 **€500~900** 실속·중간급 목표. 프리미엄 아파트·부티크호텔은 €900~1,300까지 가능. 실시간 총액 아님 |
| 필수 | Fourvière 전망·Vieux Lyon·traboules·Presqu’île·Croix-Rousse·Halles Paul Bocuse 또는 전통시장 |
| 당일치기 | **9/24 Annecy** — 구시가지·운하·호숫가·1시간 크루즈 선택 |
| 미술관 | 기본 일정에서 제외. 비·피로 시 Musée des Beaux-Arts 또는 Musée des Confluences 중 1곳만 |
| 음식 | bouchon 2회 이하. quenelle·salade lyonnaise·saucisson brioché·cervelle de canut·praline tart 중 선택 |
| 운동 | Jason: Rhône 강변 또는 Parc de la Tête d’Or 러닝 1회. Julia: LOU Piscine 선택 수영 |
| 치안 운영 | 역·관광지·혼잡한 차량에서 지퍼형 크로스백, 휴대전화 뒷주머니 금지. 수하물 도착일은 Part-Dieu→숙소 택시 권장 |
| 피로도 | 도시일 2~3, Annecy 4, 이동일 3~4 |

### 일정의 핵심 논리

```text
9/21 Avignon TGV 차량반납 → Lyon Part-Dieu → Ainay·Presqu’île 적응
        ↓
9/22 Fourvière → Vieux Lyon → traboules → Saône·Presqu’île
        ↓
9/23 Croix-Rousse 시장·비단동네 → Halles Paul Bocuse → Parc de la Tête d’Or
        ↓
9/24 Lyon Part-Dieu ↔ Annecy — 구시가지·호숫가·크루즈
        ↓
9/25 아침 생활권 산책 → Part-Dieu → Paris
```

---

## 2. 시각요소 자리표시자

### 지도 브리프 리옹 4박 전체 생활권과 동선 지도
- 설명: Part-Dieu역, Ainay·Bellecour 숙소권, Fourvière, Vieux Lyon, Presqu’île, Croix-Rousse, Halles Paul Bocuse, Parc de la Tête d’Or를 한 장에 표시한다.

### 지도 브리프 숙소 권역과 야간 귀가 안전 동선 지도
- 설명: Ainay–Ampère, Bellecour–Célestins, Brotteaux–Masséna, Part-Dieu 서쪽의 추천범위와 밝은 대로·메트로역·택시 하차점을 표시한다. Gabriel-Péri 주변과 역 바로 앞 복잡한 구간은 숙소 1순위에서 제외한다.

### 지도 브리프 Fourvière–Vieux Lyon–Presqu’île 도보 하산 지도
- 설명: 푸니쿨라 상행, Fourvière 전망대, 로마극장 선택, Jardin du Rosaire 하산, Saint-Jean·traboules·Saône 강변으로 이어지는 일방향 동선.

### 지도 브리프 Croix-Rousse–Halles–Parc de la Tête d’Or 이동 지도
- 설명: Croix-Rousse 시장과 비단노동자 동네, Halles Paul Bocuse 점심, Parc de la Tête d’Or를 대중교통으로 연결한다.

### 지도 브리프 Lyon–Annecy 철도 당일치기 지도
- 설명: Lyon Part-Dieu–Annecy 철도축과 Annecy역–구시가지–Jardins de l’Europe–Pont des Amours–Pâquier–크루즈 선착장 동선을 표시한다.

### 도식 브리프 리옹의 도시 발전 도식
- 설명: Fourvière의 로마도시 → Vieux Lyon의 중세·르네상스 → Presqu’île의 상업도시 → Croix-Rousse의 비단산업 → Part-Dieu·Confluence의 현대도시로 이어지는 구조.

---

## 3. 날짜별 확정 일정

| 날짜 | 테마 | 핵심 방문지 | 피로도 |
|---|---|---|---:|
| 9/21 월 | 도착과 두 강의 도시 적응 | Ainay·Bellecour·Jacobins·Saône 강변 | 3/5 |
| 9/22 화 | 리옹의 역사축 | Fourvière·Vieux Lyon·traboules·Presqu’île | 4/5 |
| 9/23 수 | 생활도시와 미식 | Croix-Rousse 시장·비단동네·Halles·Parc | 3/5 |
| 9/24 목 | 안시 당일치기 | Vieille Ville·Thiou 운하·호수·크루즈 | 4/5 |
| 9/25 금 | 파리 이동 | 시장 또는 강변 짧은 산책·Part-Dieu TGV | 3/5 |

---

## 4. 치안 판단과 여행 설계 반영

### 4.1 “프랑스 범죄율 1위”라는 주장에 대한 판단

프랑스 내무부 산하 SSMSI는 2025년 기준 시·구 단위로 **14개 범죄·위반 범주를 개별 지표**로 공개한다. 이 데이터에는 리옹의 각 arrondissement도 포함되지만, 정부가 범주별 수치를 하나로 합쳐 “가장 위험한 도시” 순위를 발표하지는 않는다. 따라서 특정 민간사이트가 범주를 선택해 합산하거나 인구 1,000명당 비율로 만든 순위는 **그 사이트의 계산방식**이지 공식 판정이 아니다. 특히 리옹처럼 관광객·통근자·철도 이용자가 많은 도시 중심부는 상주인구만 분모로 쓰면 절도 지표가 높게 보일 수 있다.

리옹은 대도시·관광도시·교통허브이므로 재산범죄와 소매치기 위험은 분명 존재한다. 여행자에게 현실적으로 중요한 위험은 다음 순서다.

1. Vieux Lyon, Bellecour, 축제·시장, 혼잡한 메트로·트램에서의 **소매치기와 휴대전화 절도**
2. Part-Dieu·Perrache 같은 큰 역에서 수하물에 집중하지 못하는 순간의 절도
3. 차량을 사용한다면 유리창 파손·차량 내 물품 절도
4. 심야에 낯선 골목이나 역 하부통로를 혼자 오래 걷는 상황

2026년 6월에는 Rhône 주정부와 경찰이 Part-Dieu에서 대규모 집중 단속을 실시했다. 이는 Part-Dieu를 여행금지 지역으로 볼 근거가 아니라, **유동인구가 많은 역·상업 허브라서 수하물과 휴대전화 관리가 특히 필요한 곳**이라는 판단을 뒷받침한다.

### 4.2 숙소계획에 반영한 원칙

- **1순위 Ainay–Ampère**: 비교적 차분한 주거·상업 혼합지역이며 Bellecour와 Perrache 사이 이동이 편하다.
- **2순위 Brotteaux–Masséna–Foch**: 주거환경과 Parc 접근이 좋고, Part-Dieu에도 접근하기 쉽다.
- **도심형 Célestins–Bellecour**: 매우 편리하지만 관광객·야간 유동이 많으므로 조용한 객실과 프런트 운영을 확인한다.
- **Part-Dieu 인근**: 역 바로 앞보다 서쪽·북서쪽의 밝은 대로에 있는 숙소를 고른다. 늦은 밤 역 주변 배회는 피한다.
- **Vieux Lyon 내부**: 아름답지만 야간소음·관광혼잡·소매치기·수하물 이동 때문에 숙소로는 후순위다.
- **Place Gabriel-Péri 바로 주변**: 활기 있는 다문화상권이지만 야간 혼잡과 노상문제로 여행자의 편안한 숙박 거점에는 적합하지 않다. 이는 Guillotière 전체를 위험지역으로 규정하는 의미가 아니다.

### 4.3 관광·교통계획에 반영한 원칙

- 수하물이 있는 도착·출발일에는 Part-Dieu와 숙소 사이 **택시 또는 공식 호출차량**을 우선한다.
- 일상 관광은 TCL 대중교통을 사용하되, 차량이 붐비면 휴대전화를 손에 들고 출입문 옆에 서지 않는다.
- 저녁식사 후 21시 전후에는 큰길과 메트로를 이용하고, 피곤하거나 와인을 마셨으면 택시로 귀가한다.
- 렌터카는 Avignon TGV에서 반납하여 리옹 시내로 가져오지 않는다. 차량이 있다면 지하주차장에 넣고 아무것도 보이게 두지 않는다.
- 숙소는 24시간 또는 최소 야간 프런트, 엘리베이터, 실내 금고, 밝은 현관을 우선한다.

---

## 5. Day 1 — 9월 21일 월요일
### Avignon TGV 차량반납, Lyon 도착, Ainay와 Presqu’île 적응

### 사진 브리프 Bellecour 광장에서 Fourvière를 바라보는 첫 풍경
- 설명: 넓은 광장 너머 언덕 위 성당이 보이며 리옹의 지형을 한 번에 이해하게 하는 도착일 이미지.

### 이동 권장안

Avignon TGV에서 렌터카를 반납한 뒤 Lyon Part-Dieu행 직행 TGV를 이용한다. SNCF 공개 시간표에는 Avignon TGV–Lyon 구간이 하루 여러 편 운행되고, 최단 약 1시간 내외의 직행편이 있다. 실제 2026년 9월 21일 편성은 예약창 개방 후 확정한다.

**권장 목표**: Avignon TGV 10시 전후 차량반납 → 10:20~11:10대 TGV → Lyon Part-Dieu 11:30~12:30 도착 범위.

| 시간대 | 일정 | 실행 포인트 |
|---|---|---|
| 오전 | Avignon TGV 렌터카 반납 | 주유·차량사진·영수증·보증금 해제절차 확인 |
| 10:20~12:30 범위 | TGV로 Lyon Part-Dieu 이동 | 열차 20분 전 승강장권 진입, 짐은 시야 안쪽에 둔다 |
| 도착 후 30~45분 | Part-Dieu→숙소 택시 | 큰 짐을 가지고 메트로 환승하지 않는다. 공식 택시승강장 이용 |
| 13:00 전후 | 체크인 또는 짐 보관 | 객실금고·엘리베이터·야간출입 확인 |
| 13:15~14:00 | 가벼운 점심 | 숙소권 빵집·샐러드·샌드위치. 도착일 부숑 코스 금지 |
| 14:30~15:30 | 숙소 휴식 | 차량반납과 기차이동 뒤 1시간 휴식 |
| 15:45~17:45 | **Ainay→Bellecour→Jacobins→Célestins** 산책 | 도시 중심방향과 귀가로를 익힌다 |
| 17:45~18:30 | Saône 강변·카페 | Fourvière·Vieux Lyon을 건너다보기만 하고 깊게 들어가지 않음 |
| 19:30~21:00 | **Café Comptoir Abel** 저녁 | quenelle 또는 poulet aux morilles를 나눠 먹고 과식 금지 |
| 21:00 이후 | 숙소 귀가 | Ainay 숙소면 큰길을 따라 도보, 피로·음주 시 택시 |

**오늘의 피로도: 3/5.** 여행일이므로 핵심은 체크인, 생활권 파악, 첫 저녁뿐이다.

### 꼭 해볼 것

- Bellecour에서 Fourvière 언덕을 올려다보며 다음 날 지형을 미리 이해하기
- Place des Jacobins와 Célestins 주변의 19세기 도심 파사드를 천천히 보기
- 첫 부숑 식사는 2개 요리만 선택하여 남은 리옹 일정의 위장 부담 줄이기

### 지연 시 삭제 순서

1. Jacobins·Célestins 산책
2. Saône 카페
3. 외식은 숙소권 간단식으로 전환

---

## 6. Day 2 — 9월 22일 화요일
### Fourvière·Vieux Lyon·traboules — 도시의 2,000년을 아래로 걸어 내려오기

### 사진 브리프 Fourvière 전망대에서 내려다본 Presqu’île와 두 강
- 설명: Saône과 Rhône, Presqu’île, Part-Dieu 고층부가 한눈에 보여 리옹의 도시구조를 설명하는 대표 이미지.

### 사진 브리프 Vieux Lyon 르네상스 안뜰과 traboule
- 설명: 거리에서 안뜰과 다른 골목으로 이어지는 반공공 통로의 공간구조를 보여주는 이미지.

| 시간 | 일정 | 실행 포인트 |
|---|---|---|
| 07:45~08:45 | 숙소 아침·준비 | 이날은 오르막 대신 푸니쿨라를 사용하므로 정식 운동 생략 |
| 09:10~09:40 | Metro D → Vieux Lyon → Funicular F2 | TCL 24시간권 또는 컨택트리스 이용 |
| 09:40~10:40 | **Basilique de Fourvière·전망대** | 성당 내부 20~30분, 전망과 도시지형 설명에 더 많은 시간 |
| 10:45~11:30 | **로마극장 외부와 언덕산책 선택** | 고고학박물관 내부는 생략. 극장·도시기원만 확인 |
| 11:35~12:15 | **Jardin du Rosaire 하산** | 계단과 경사 주의. 비가 오면 푸니쿨라로 되돌아감 |
| 12:20~13:10 | Vieux Lyon 가벼운 점심 | salade lyonnaise 또는 샌드위치. 관광지 메인광장보다 골목 외곽 선택 |
| 13:15~15:15 | **Saint-Jean·traboules 자율산책** | 공개된 통로만 이용하고 주민공간에서 조용히 이동 |
| 15:15~16:00 | Saône 강변·Passerelle du Palais de Justice | 구시가지 파사드와 Fourvière를 강 건너에서 조망 |
| 16:00~17:15 | Presqu’île 카페·자유 | 피로하면 숙소 귀환. 쇼핑은 Rue de la République 30분 이내 |
| 17:30~18:45 | 숙소 휴식 | 저녁 전 최소 1시간 휴식 |
| 19:30~21:15 | **Daniel et Denise** 특별 저녁 | pâté en croûte, quenelle de brochet sauce Nantua, tarte praline 중 공유 |
| 21:15 이후 | 택시 또는 메트로 귀가 | 휴대전화·지갑을 지퍼가방 안쪽에 넣는다 |

**오늘의 피로도: 4/5.** 예상 보행 8~10km, 하산 계단 포함.

### 오늘의 필수·선택

- **필수**: Fourvière 전망, Vieux Lyon, traboule 2~4개
- **우선 추천**: Jardin du Rosaire 하산, Saône 강변
- **선택**: 로마극장, Saint-Jean 성당 내부
- **대체**: 비가 오면 Fourvière→푸니쿨라 하산→Musée Gadagne 또는 Musée des Beaux-Arts 중 1곳

### 꼭 해볼 것

- Fourvière에서 Rhône과 Saône, Presqu’île, Part-Dieu의 위치를 직접 비교하기
- traboule을 “비밀통로 수집”이 아니라 상업도시와 주거건축의 구조로 이해하기
- Vieux Lyon에서 휴대전화로 지도를 계속 보며 걷지 말고, 안전한 광장에서 동선을 확인한 뒤 이동하기

---

## 7. Day 3 — 9월 23일 수요일
### Croix-Rousse 시장·비단동네, Halles Paul Bocuse와 도시공원

### 사진 브리프 Boulevard de la Croix-Rousse 아침시장
- 설명: 치즈·과일·채소와 지역 주민이 어우러지는 생활형 시장 풍경.

### 사진 브리프 Croix-Rousse의 canut 계단과 벽화
- 설명: 높은 천장의 비단직조 작업장과 경사면 계단이 만든 노동도시의 모습을 보여주는 이미지.

### 사진 브리프 Halles Paul Bocuse의 치즈·샤퀴테리 진열
- 설명: 리옹이 미식도시로 불리는 이유를 식재료의 밀도로 보여주는 이미지.

| 시간 | 일정 | 실행 포인트 |
|---|---|---|
| 07:15~08:00 | Jason 선택 러닝 | Rhône 강변 5~7km. 전날 무릎피로가 있으면 생략 |
| 08:00~09:00 | 숙소 아침·샤워 | 시장에서 간식·과일 구매 예정 |
| 09:20~10:10 | **Croix-Rousse 시장** | 수요일 식품시장 06:00~13:00. 과일·치즈·빵 소량 구매 |
| 10:15~12:00 | **Croix-Rousse 평지와 slopes** | Place de la Croix-Rousse→Maison des Canuts 외관→Mur des Canuts 선택→경사면 traboule |
| 12:00~12:30 | 대중교통으로 Halles 이동 | C선·메트로·트램 조합, 혼잡 시 택시 |
| 12:35~14:05 | **Halles de Lyon Paul Bocuse 점심** | Giraudet quenelle, Sibilia charcuterie, Mère Richard cheese 등 2~3곳을 나눠 맛봄 |
| 14:15~15:00 | 숙소 또는 카페 휴식 | 과식했으면 공원 이동을 30분 늦춤 |
| 15:15~17:15 | **Parc de la Tête d’Or** | 호수, 장미원·식물원 외부, 벤치 휴식. Jason 짧은 스케치 가능 |
| 17:15~18:00 | Julia 선택 수영 또는 공원 카페 | Julia는 LOU Piscine으로 이동하기보다 이날 공원 휴식을 기본으로 함 |
| 18:30~19:15 | 숙소 귀환·휴식 | 저녁은 가볍게 |
| 19:45~21:00 | 가벼운 저녁 | cervelle de canut·샐러드·생선 등. 부숑 3회째는 피함 |

**오늘의 피로도: 3/5.** 시장·언덕·실내시장·공원이 성격이 달라 지루하지 않지만, 이동이 많다.

### 꼭 해볼 것

- Croix-Rousse를 전망지가 아니라 비단노동자의 주거·작업도시로 보기
- Halles에서는 한 식당에 오래 앉기보다 지역 식재료를 2~3종 비교하기
- Parc de la Tête d’Or에서 장기여행의 완충시간을 확보하기

### 선택 모듈

- **도시경관 우선**: Croix-Rousse에서 Saône 방향으로 천천히 하산
- **현대도시 우선**: 공원 대신 Confluence와 Navigône 수상교통
- **비 오는 날**: Halles 체류 연장 + Musée des Beaux-Arts 2시간
- **Julia 수영**: LOU Piscine 25m 실내풀 또는 50m 야외풀의 당일 자유수영 시간을 별도 확인하여 공원 대신 배치

---

## 8. Day 4 — 9월 24일 목요일
### Annecy 당일치기 — 운하 구시가지와 알프스 호수

### 사진 브리프 Palais de l’Île와 Thiou 운하
- 설명: 물길 사이의 석조건물, 파스텔색 구시가지와 산 배경이 겹치는 안시의 대표 이미지.

### 사진 브리프 Pont des Amours에서 본 Annecy 호수
- 설명: Jardins de l’Europe, 호수, 산맥이 연결되는 안시의 핵심 풍경.

### 사진 브리프 Annecy 호수 1시간 크루즈
- 설명: 도시 산책만으로는 보이지 않는 호수 주변 산악지형을 수면에서 이해하게 하는 이미지.

SNCF Connect의 현재 노선 안내는 Lyon–Annecy에 하루 약 **19편**, 평균 약 **2시간 7분**, 최단 약 **1시간 51분**이 걸린다고 안내한다. 직행편과 환승편이 섞여 있으므로 **2026년 9월 24일에는 07:30~08:30 Part-Dieu 출발의 직행 TER를 우선**하고, 실제 편성·공사·파업 여부를 예약 시점과 전날 다시 확인한다.

| 시간 | 일정 | 실행 포인트 |
|---|---|---|
| 06:40~07:20 | 숙소 아침·Part-Dieu 이동 | 수하물 없이 작은 지퍼가방만 휴대. 택시 또는 메트로로 역 이동 |
| 07:40 이전 | Part-Dieu 도착 | 열차 20~25분 전 플랫폼 확인 |
| 07:30~08:30 출발 목표 | **Lyon→Annecy 직행 TER 우선** | 소요 약 1시간 50분~2시간 10분. 실제 9/24 시간표 재확인 |
| 10:15~11:45 | **Vieille Ville·Thiou 운하** | 역→Rue Sainte-Claire→Palais de l’Île 외관→운하 골목 |
| 11:50~13:05 | **Savoy 점심** | Chez Mamie Lise에서 diots·polenta 또는 일일메뉴. 무거운 fondue는 2인 공유 또는 생략 |
| 13:10~14:10 | **Jardins de l’Europe·Pont des Amours·Pâquier** | 호수와 산의 방향을 이해하는 핵심 산책 |
| 14:30~15:30 | **1시간 호수크루즈 선택** | 공식 안내 계획가 성인 €19. 날씨·운항 확인 |
| 15:40~17:15 | 호숫가 자유시간·카페 | 크루즈를 생략하면 Imperial 방향 평지산책 또는 Palais de l’Île 내부 선택 |
| 17:20~18:00 | 역 이동·간단한 간식 | 귀환열차 20분 전 도착 |
| 18:00~19:00대 | Annecy→Lyon 귀환 | 직행 우선, 실제 시각 재확인 |
| 20:00~21:00 | Lyon 도착·숙소 귀환 | 역에서 바로 택시. 저녁은 숙소권 간단식 |

**오늘의 피로도: 4/5.** 왕복 철도 약 4시간과 관광 7시간.

### 중요한 요일 정보

Annecy **구시가지 시장**은 화·금·일 07:00~13:00에 열리므로 목요일 9월 24일에는 열리지 않는다. 목요일에는 Novel의 Avenue de France와 Cran-Gevrier의 Place Chorus 등 다른 생활권 시장이 열리지만, 구시가지·호수 동선에서 벗어나므로 일부러 찾아가지 않는다. 이날은 운하·호숫가·크루즈에 집중한다.

### 안시에서 꼭 해볼 것

- Thiou 운하를 따라 골목과 물길이 도시를 어떻게 나누는지 보기
- Palais de l’Île는 외관을 우선하고, 내부는 비·피로 상황에 따라 선택
- Pont des Amours와 Pâquier에서 호수와 산악지형을 길게 바라보기
- Savoy 음식은 fondue 하나에 집중하기보다 diots·polenta, tartiflette, 지역치즈 중 몸 상태에 맞게 선택
- 날씨가 좋으면 1시간 크루즈로 호수도시의 규모를 체감하기

### 점심 후보

| 식당 | 성격 | 추천메뉴·계획가격 | 예약 |
|---|---|---|---|
| **Chez Mamie Lise** | 구시가지 Savoy 전통식 | 일일요리 약 €19, 메뉴 약 €32.90, diots 약 €26.80, tartiflette 약 €25.50 | 점심 권장 |
| **Le Freti** | 치즈요리 전문 | fondue, raclette, tartiflette | 점심 예약 권장 |
| 가벼운 대안 | 빵집·샌드위치·샐러드 | 크루즈 전 과식 방지 | 불필요 |

### 우천·지연 대체안

- 비: Vieille Ville→Palais de l’Île 내부→긴 점심→비가 약해지면 호숫가. 크루즈는 운항 시에만
- 강풍·크루즈 취소: Pâquier·Imperial 구간 평지산책과 카페로 대체
- 열차 지연 60분 이상: 크루즈를 삭제하고 구시가지+호숫가만
- 대규모 철도장애: Lyon의 Confluence·Navigône·Parc 생활일로 대체

---

## 9. Day 5 — 9월 25일 금요일
### 생활권의 마지막 아침, Part-Dieu에서 파리 이동

| 상대시간 | 일정 | 실행 포인트 |
|---|---|---|
| 기차 4시간 전 | 기상·숙소 아침·짐정리 | 여권·티켓·귀중품은 몸에 지닌 작은 가방으로 분리 |
| 기차 3시간 전 | 선택: Saint-Antoine 시장 또는 Rhône 강변 30분 | 열차가 11시 이후일 때만. 시장은 금요일 오전 운영 |
| 기차 2시간 전 | 체크아웃 | 숙소 영수증·분실물 확인 |
| 기차 90분 전 | 숙소→Part-Dieu 택시 | 큰 짐이므로 메트로 환승보다 택시 우선 |
| 기차 30분 전 | 플랫폼 부근 대기 | 카페에 짐을 흩어두지 않고, 캐리어 손잡이를 잡거나 다리 사이에 둔다 |
| 탑승 | Lyon→Paris TGV | 짐 선반이 시야 밖이면 자물쇠·와이어스트랩 사용 고려 |

**오늘의 피로도: 3/5.** 파리 체크인까지 이어지는 이동일이므로 리옹에서 새로운 관광을 추가하지 않는다.

---

## 10. 피로도·삭제 우선순위

| 날짜 | 피로도 | 삭제 1순위 | 삭제 2순위 |
|---|---:|---|---|
| 9/21 | 3 | Jacobins·Célestins 추가산책 | 외식 |
| 9/22 | 4 | 로마극장 | Saint-Jean 내부 |
| 9/23 | 3 | Parc 또는 Confluence 선택모듈 | 러닝 |
| 9/24 | 4 | 크루즈 | Palais de l’Île 내부 |
| 9/25 | 3 | 시장·강변산책 | 없음—이동 우선 |

---

## 11. 문전 이동과 교통 실행표

| 구간 | 표면 이동 | 현실적 문전시간 | 권장방식 |
|---|---:|---:|---|
| Avignon TGV→Lyon Part-Dieu | 직행 최단 약 1시간 | 반납·대기 포함 2.5~3.5시간 | TGV 직행 |
| Part-Dieu→Ainay 숙소 | 차 15~25분 | 택시대기 포함 25~40분 | 수하물 있는 날 택시 |
| Ainay→Vieux Lyon | Metro D 또는 도보 | 20~35분 | 낮에는 대중교통·도보 |
| Vieux Lyon→Fourvière | 푸니쿨라 수분 | 대기 포함 15~25분 | F2 기본 |
| 숙소→Croix-Rousse | 메트로 A+C | 30~45분 | 출근혼잡 피함 |
| 숙소→Part-Dieu | 메트로·트램 20~35분 | 짐 있으면 35~50분 | 출발일 택시 |
| Lyon→Annecy | 열차 약 1시간 50분~2시간 20분 | 숙소 문전 약 2시간 30분 | 직행 TER 우선 |

### TCL 이용 판단

- Zone 1+2 1회권은 계획기준 약 €2.10
- 24시간권은 계획기준 약 €6.90
- 컨택트리스 은행카드로 여러 번 탑승하면 일일 상한이 적용되는 방식이 편리할 수 있다.
- Fourvière 푸니쿨라는 Zone 1 유효권·패스에 포함되는 방식이 기본이다.
- 하루 3회 이상 타는 날은 24시간권 또는 컨택트리스 상한이 편하다.

---

## 12. 예약 우선순위

| 우선순위 | 항목 | 권장일시 | 확인사항 |
|---:|---|---|---|
| P0 | Lyon 숙소 | 9/21~9/25 | 동네·야간 프런트·엘리베이터·냉방·소음·총액 |
| P0 | Avignon TGV→Lyon | 9/21 오전 | 차량반납 여유 60~90분, 직행편 |
| P0 | Lyon→Paris | 9/25 | 숙소 체크아웃과 Part-Dieu 도착 여유 |
| P1 | Lyon↔Annecy | 9/24 | 직행·좌석·마지막 귀환편·공사 |
| P1 | Daniel et Denise | 9/22 19:30 | 휴무·메뉴·지각정책 |
| P1 | Annecy 점심 | 9/24 11:50~12:00 | Chez Mamie Lise 또는 Le Freti |
| P2 | Annecy 크루즈 | 9/24 14:30 전후 | 계절 운항·날씨·선착장 |
| P2 | Café Comptoir Abel | 9/21 19:30 | 도착 지연을 고려한 취소정책 |
| P3 | LOU Piscine | 선택 | 자유수영 레인·운영시간·수영모 규정 |

---

# Layer 2. 지역 이해 · 동네 · 추천등급 · 방문지 설명

## 13. 리옹을 이해하는 다섯 개의 도시층

### 13.1 Fourvière — 로마도시와 종교언덕

리옹의 시작은 Saône과 Rhône이 만나는 평지가 아니라 Fourvière 언덕의 로마도시 Lugdunum이다. 언덕에서 내려다보면 이후 도시가 두 강 사이와 동쪽 평야로 확장된 과정을 읽을 수 있다.

### 13.2 Vieux Lyon — 르네상스 국제상업도시

Saône 서안의 Vieux Lyon은 약 24ha에 걸친 유럽 최대급 르네상스 도시구역 중 하나다. 이탈리아 상인과 금융가, 성직자, 인쇄업자가 모였고, 안뜰과 traboule은 높은 밀도의 상업·주거공간을 연결했다.

### 13.3 Presqu’île — 광장과 상업의 중심

Saône과 Rhône 사이 반도는 Bellecour, Jacobins, Terreaux를 잇는 리옹의 중심축이다. 걷기 쉽고 저녁식사와 쇼핑이 편하지만, 관광객과 유동인구가 많아 가방관리가 필요하다.

### 13.4 Croix-Rousse — 비단노동자의 언덕

Croix-Rousse의 높은 천장 주택과 경사면 통로는 canut라 불린 비단직조 노동자의 작업·생활 조건에서 생겼다. 화려한 비단상품보다 노동도시의 건축과 시장을 보는 것이 중요하다.

### 13.5 Part-Dieu·Confluence — 현대도시의 두 방향

Part-Dieu는 철도·업무·상업 허브이고, Confluence는 산업지에서 문화·주거·수변도시로 재개발된 구역이다. 이번 4박에는 Part-Dieu는 교통거점, Confluence는 선택 산책으로만 다룬다.

---

## 14. 동네별 성격·숙소 적합성·안전 고려

| 권역 | 성격과 장점 | 밤·안전 고려 | Jason·Julia 적합성 | 추천 |
|---|---|---|---|---|
| **Ainay–Ampère** | 조용한 2구 생활권, Bellecour·Perrache 사이, Saône 산책 | 대체로 차분. Perrache 하부통로는 늦은 밤 불필요하게 걷지 않음 | 생활·식당·교통 균형 | **1순위** |
| **Bellecour 남부–Célestins** | 중심관광·식당·광장 접근 최고 | 혼잡·소매치기·주말 소음, 안뜰객실 선호 | 짧은 체류 매우 편리 | **1.5순위** |
| **Brotteaux–Masséna–Foch** | 6구 주거환경, Parc·Part-Dieu 접근 | 비교적 안정적, 역까지 밝은 길 선택 | 조용함·운동 우수 | **2순위** |
| **Part-Dieu 서·북서** | TGV·Halles·교통 편리 | 역 주변 혼잡. 24h 프런트와 큰길 동선 필수 | 당일치기·파리 이동 최적 | **조건부** |
| **Confluence** | 현대적·수변·상대적으로 조용 | 중심관광까지 이동, 늦은 밤 수변 외진 구간 피함 | 아파트호텔·휴식형 | 선택 |
| **Vieux Lyon 내부** | 역사적 분위기 최고 | 관광혼잡·소음·계단·소매치기 | 관광은 좋지만 숙박은 불리 | 후순위 |
| **Gabriel-Péri 바로 주변** | 교통·다문화 음식·야간활기 | 노상혼잡과 체감불안에 대한 현지 우려가 반복됨 | 조용한 장기여행 숙소에는 부적합 | 비추천 |

### 숙소의 필수 확인사항

- 22시 이후 현관 출입방식과 야간 프런트
- 객실금고와 캐리어 보관공간
- 안뜰 또는 고층 조용한 객실
- 엘리베이터와 계단 수
- Metro 역에서 숙소까지 조명이 밝은 5~10분 동선
- 9/25 체크아웃 후 짐 보관 또는 택시 호출 지원

---

## 15. 추천등급

| 장소 | 등급 | 이유 |
|---|---|---|
| Fourvière 전망·성당 | **필수** | 도시 지형과 2,000년 역사를 한 번에 이해 |
| Vieux Lyon·traboules | **필수** | 르네상스 상업도시의 공간구조 |
| Presqu’île·Bellecour·Jacobins | **필수** | 리옹의 현재 중심과 강 사이 도시체험 |
| Croix-Rousse 시장·동네 | **우선 추천** | 비단노동자 역사와 생활시장 |
| Halles Paul Bocuse | **우선 추천** | 미식도시를 음식점이 아니라 재료로 이해 |
| Parc de la Tête d’Or | **우선 추천** | 장기여행의 운동·휴식·스케치 완충지 |
| Annecy | **필수에 가까운 우선 추천** | 리옹과 성격이 완전히 다른 알프스 호수도시 |
| Saône·Rhône 강변 | **우선 추천** | 도시의 두 강과 생활동선을 경험 |
| Confluence·Navigône | 선택 | 현대도시·수변재생에 관심 있을 때 |
| Musée des Beaux-Arts | 대체 | 우천 시 2시간, 종합미술관 관심이 있을 때만 |
| Musée des Confluences | 대체 | 비·현대건축·인류학 관심 시 |
| Institut Lumière | 선택 | 영화사 관심이 클 때 |
| Beaujolais 당일치기 | 비추천 | 4박에서 Annecy와 리옹 자체를 약화 |
| 역 주변 야간산책 | 비추천 | 여행가치 없이 피로·체감불안만 증가 |

---

## 16. 주요 방문지 상세 소개

### 16.1 Fourvière — 리옹을 보기 전에 도시를 읽는 곳

### 사진 브리프 Fourvière에서 두 강과 Part-Dieu를 조망하는 파노라마
- 설명: 고대 언덕도시에서 현대 고층업무지구까지 한 화면에 담는다.

**왜 추천하는가**: 리옹은 평면지도로만 보면 이해하기 어렵다. Fourvière에서 보면 Saône, Presqu’île, Rhône, Part-Dieu의 순서가 즉시 보인다.

**꼭 해볼 것**
- 전망대에서 두 강의 위치를 구분하기
- 성당 내부의 장식은 20~30분만 보고 도시전망에 집중하기
- 비가 오지 않으면 Jardin du Rosaire로 하산하기

**적정 체류**: 60~90분

**주의**: 휴대전화를 난간에 올려두지 않고, 계단 하산 시 무릎보호.

---

### 16.2 Vieux Lyon과 traboules — 골목 안쪽의 도시

### 사진 브리프 Renaissance courtyard와 나선계단
- 설명: 좁은 파사드 뒤에 넓은 안뜰과 계단탑이 숨겨진 리옹의 건축특성을 보여준다.

**왜 추천하는가**: 외관만 보면 비슷한 구시가지이지만, traboule과 안뜰을 보면 국제상업도시의 높은 밀도와 동선을 이해할 수 있다.

**꼭 해볼 것**
- Saint-Jean 구역의 공개 traboule 2~4개만 선택
- 거리→안뜰→다른 거리로 이어지는 구조를 확인
- Saône 건너편에서 Vieux Lyon 파사드를 다시 보기

**적정 체류**: 2~2.5시간

**예절·안전**: 일부 통로는 실제 주거공간이다. 조용히 이동하고 잠긴 문을 억지로 열지 않는다. 군중 속에서는 지퍼가방을 앞쪽에 둔다.

---

### 16.3 Presqu’île — 광장으로 읽는 리옹 중심

### 사진 브리프 Place des Jacobins 분수와 주변 파사드
- 설명: 리옹의 상업·도시미를 대표하는 광장 장면.

**꼭 해볼 것**
- Bellecour에서 Fourvière 방향 조망
- Jacobins·Célestins·République를 연결해 걷기
- Saône 또는 Rhône 강변 중 한쪽을 30분 걷기

**적정 체류**: 2~3시간을 여러 날 분산

**안전**: 쇼핑거리·메트로 출입구에서 휴대전화를 손에 든 채 멈춰 서지 않는다.

---

### 16.4 Croix-Rousse — 비단보다 사람과 공간을 보기

### 사진 브리프 canut 주택의 높은 창과 경사로
- 설명: 대형 직조기를 수용하기 위한 높은 천장과 언덕의 경사가 결합한 도시풍경.

**꼭 해볼 것**
- 수요일 시장에서 과일·치즈·빵 구매
- 경사면 traboule 또는 escaliers 1개만 천천히 내려가기
- Mur des Canuts는 동선이 맞을 때 선택

**적정 체류**: 2.5~3시간

---

### 16.5 Halles de Lyon Paul Bocuse — 미식도시의 진열장

### 사진 브리프 quenelle·소시지·치즈가 진열된 Halles 내부
- 설명: 리옹의 대표 음식이 실제 재료와 전문점으로 분화된 모습을 보여준다.

**꼭 해볼 것**
- quenelle, charcuterie, Saint-Marcellin 또는 praline 디저트 중 2~3개 선택
- 선물보다 당일 먹을 소량구매
- 점심 피크를 피하려면 12:30 이전 도착

**적정 체류**: 60~90분

**예산**: 두 사람이 여러 점포를 나눠 맛보면 €35~70 계획.

---

### 16.6 Parc de la Tête d’Or — 장기여행의 회복공간

### 사진 브리프 호숫가 산책로와 나무그늘
- 설명: 도시관광 사이의 휴식과 운동을 상징하는 이미지.

**꼭 해볼 것**
- 호수 주변 평지산책
- Jason은 20~40분 스케치 또는 러닝
- 벤치에서 30분 이상 아무 일정 없이 쉬기

**적정 체류**: 1.5~2시간

---

### 16.7 Annecy — 알프스 호수를 품은 물의 도시

### 사진 브리프 운하·파스텔색 건물·산 배경이 겹친 Vieille Ville
- 설명: 중세구시가지와 알프스 자연이 동시에 느껴지는 안시의 정체성.

**왜 추천하는가**: 리옹의 도시·음식·산업 경험 뒤에 호수와 산악지형을 추가해 4박의 다양성을 크게 높인다. 박물관 당일치기가 아니라 **구시가지·운하·호숫가·배**를 경험하는 날이다.

**꼭 해볼 것**
- Sainte-Claire 거리와 Thiou 운하 산책
- Pont des Amours에서 호수와 산 조망
- 날씨가 좋으면 1시간 크루즈
- Savoy 음식 1종 체험

**적정 체류**: 6~7시간

---

## 17. 미술관·박물관 선택 기준

이번 챕터에서는 실내시설을 기본 일정에서 제외한다.

| 시설 | 넣는 조건 | 적정시간 |
|---|---|---:|
| Musée des Beaux-Arts | 비·추위, 종합회화 관심, 9/23 공원 대체 | 2시간 |
| Musée des Confluences | 현대건축·인류학 관심, Confluence 방문 | 2~2.5시간 |
| Musée Gadagne | Vieux Lyon 우천, 도시사 관심 | 1.5~2시간 |
| Institut Lumière | 영화사 관심이 매우 클 때 | 1.5시간 |
| Palais de l’Île, Annecy | 비·크루즈 취소 | 45~60분 |

---

# Layer 3. 숙소 · 음식 · 시장 · 운동 · 이벤트 · 안전 · 예약

## 18. 실제 숙소 후보

> 아래는 위치·시설·공식 안내를 기준으로 한 1차 후보이며, 2026년 9월 21~25일 실시간 총액은 예약 시 재확인한다. 수영장은 숙소평가 요소가 아니다.

| 후보 | 권역·형태 | 장점 | 약점·확인사항 | 적합도 |
|---|---|---|---|---:|
| **Citadines Presqu’île Lyon** | Presqu’île, 아파트호텔 | kitchenette, 중심도보, 엘리베이터, 주방형 4박 적합 | 관광객 많은 중심부, 조용한 안뜰객실 요청 | **92/100** |
| **Hôtel Vaubecour** | Ainay, 소형호텔 | Saône 100m, 아래층 minimarket, 생활권·가격 균형 | 엘리베이터 전 2개 계단, 객실크기 확인 | **91/100** |
| **Warwick Reine Astrid** | 6구·Tête d’Or, 레지던스 | kitchenette, 냉방, 세탁실, 차분한 주거권 | Vieux Lyon까지 이동, 총액 높을 수 있음 | **90/100** |
| **Hôtel des Artistes** | Célestins, 호텔 | 중심인데 비교적 조용, 엘리베이터·냉방 | 주방 없음, 행사일 광장소음 확인 | **88/100** |
| **Hôtel de l’Abbaye** | Ainay, 부티크호텔 | Bellecour·Perrache 사이, 야간 프런트, 분위기 우수 | 가격 높음, 주방 없음 | **87/100** |
| **Adagio Lyon Patio Confluence** | Perrache–Confluence, 아파트호텔 | 주방, 전용주차, 피트니스, 넓은 객실 가능 | 중심 북부관광까지 이동, 역 하부 귀가동선 확인 | **84/100** |
| **Hotelo Lyon Ainay** | Perrache·Ainay, 실속호텔 | 가격, 방음, 역·Ainay 접근 | 일부 객실 냉방조건, 주차비·주방 없음 | **82/100** |
| **Citadines Part-Dieu** | Part-Dieu, 아파트호텔 | Annecy·Paris 철도, 주방·세탁 | 역주변 혼잡과 도심관광 이동, 야간 큰길 동선 필수 | **80/100** |

### 최종 선택 권고

1. **Hôtel Vaubecour 또는 Citadines Presqu’île**가 총액 €500~800이면 최우선 비교
2. 조용한 주거환경과 넓은 객실을 원하면 **Warwick Reine Astrid**
3. 중심 호텔형이면 **Hôtel des Artistes**, 특별 숙박이면 **Hôtel de l’Abbaye**
4. Annecy와 Paris 기차 편의만으로 Part-Dieu 바로 앞을 선택하지 않는다. 수하물일 2회는 택시로 해결하는 편이 낫다.

### 숙소 문의문 핵심

- 22시 이후 프런트와 현관 출입방식
- 안뜰 또는 거리소음이 적은 객실 가능 여부
- 객실금고·냉방·엘리베이터
- Metro역에서 가장 밝고 단순한 귀가경로
- 체크아웃 후 짐 보관·택시 호출 가능 여부
- 관광세·청소비·취소조건을 포함한 최종 총액

---

## 19. 레스토랑·음식·주문가이드

### 사진 브리프 quenelle de brochet sauce Nantua
- 설명: 부드러운 생선반죽과 갑각류 소스가 결합된 리옹 대표요리.

### 사진 브리프 bouchon의 salade lyonnaise와 pâté en croûte
- 설명: 리옹 음식이 풍성하지만 무거울 수 있음을 보여주는 전통 상차림.

### 19.1 저녁식당 후보

| 식당 | 성격 | 추천메뉴 | 계획가격 | 예약 |
|---|---|---|---:|---|
| **Café Comptoir Abel** | Ainay 인근 역사적 전통식당 | quenelle, poulet aux morilles, cervelle de canut | 메인 €20~30대 | 권장 |
| **Daniel et Denise Part-Dieu** | Joseph Viola의 정통 bouchon | pâté en croûte, quenelle Nantua, praline 디저트 | 점심 €21, 계절메뉴 약 €41 | **권장** |
| **Daniel et Denise Croix-Rousse** | 시장·언덕 일정과 연결 | 전통 리옹 메뉴 | 메뉴 약 €33, 시식메뉴 약 €41 | 권장 |
| **Halles Paul Bocuse 점포** | 시장형 점심 | Giraudet quenelle, Sibilia charcuterie, Mère Richard 치즈 | 2인 €35~70 | 불필요·혼잡 대비 |
| **L’Artichaut** | Hôtel de l’Abbaye 내 현대 비스트로 | 제철 프랑스요리 | 공식메뉴 재확인 | 숙박 시 권장 |

### 19.2 음식 주문 원칙

리옹 음식은 내장, 버터, 소스, 샤퀴테리 비중이 높다. 네 번의 저녁을 모두 bouchon으로 채우지 않는다.

| 메뉴 | 설명 | 이번 여행에서의 판단 |
|---|---|---|
| quenelle de brochet | 민물생선 무스 형태, Nantua 소스 | **필수 1회** |
| salade lyonnaise | frisée, 달걀, lardons | 가벼워 보이나 짠맛·지방 유의 |
| pâté en croûte | 파이껍질 속 육류·젤리 | 2인이 전채로 공유 |
| saucisson brioché | 소시지를 브리오슈에 구움 | 점심 또는 공유 |
| cervelle de canut | 허브를 섞은 fromage blanc | 가벼운 전채로 추천 |
| tablier de sapeur | 양 위 튀김 | 내장 선호가 분명할 때만 |
| tarte à la praline | 진한 분홍색 프랄린 타르트 | 커피와 작은 조각 공유 |

### 식사 배치

- 9/21: Café Comptoir Abel — 대표요리 1회
- 9/22: Daniel et Denise — 특별 부숑 저녁
- 9/23: Halles 점심 후 저녁은 가벼운 비스트로
- 9/24: Annecy Savoy 점심, Lyon 저녁은 숙소권 간단식

---

## 20. 시장·슈퍼·빵·치즈·제철 식재료

### 20.1 시장

| 시장 | 운영계획 | 역할 |
|---|---|---|
| **Croix-Rousse 식품시장** | 수요일·목요일 06:00~13:00, 다른 요일 더 큰 규모 | 9/23 생활시장 핵심 |
| **Marché Saint-Antoine** | 화~목 오전, 금·토 오후 13:30까지, 일 14:00까지 | 숙소가 Ainay·Presqu’île이면 장보기 |
| **Halles Paul Bocuse** | 점포별 상이 | 지역식재료 체험과 점심 |

### 20.2 숙소에서 살 것

- baguette 또는 pain de campagne
- jambon cru·rosette de Lyon 소량
- Saint-Marcellin 또는 지역 chèvre
- 토마토·샐러드·포도·배·무화과
- 요거트·물·견과류
- Annecy 열차용 샌드위치 재료

### 20.3 9월 제철

- 포도, 무화과, 배, 사과
- 토마토, 가지, courgette, 버섯
- 지역치즈와 charcuterie는 매일 대량 구매하지 않고 1~2일분만

---

## 21. Jason 운동·Julia 선택 수영

### 21.1 Jason 러닝

**A — Rhône 강변 5~8km**

```text
Ainay/Bellecour 숙소
→ Pont de la Guillotière 또는 Pont Wilson
→ Rhône 동쪽 강변 북상
→ Parc de la Tête d’Or 남단 또는 적정지점 회차
→ 숙소
```

- 평탄하고 거리조절이 쉽다.
- 07:00~08:00 권장.
- 자전거·전동킥보드와 보행로 구분 주의.

**B — Parc de la Tête d’Or 4~7km**

- 호수와 큰길을 조합.
- 숙소가 6구일 때 최적.

### 21.2 Julia 수영 — 선택

**LOU Piscine, Gerland**

- 50m 야외풀과 25m 실내풀을 갖춘 선택지
- Metro B Stade de Gerland 접근
- 성인 일일요금 계획기준 약 €6.50
- 9월 2026 자유수영 레인과 운영시간을 출발 전 재확인

**Piscine Garibaldi**

- 약 33m, 5레인
- 중심 접근은 좋지만 공사·계절 운영 가능성을 재확인

> 수영은 숙소선정 조건이 아니며, Annecy 당일치기와 도시관광을 희생해서 넣지 않는다.

---

## 22. 2026 행사·특별 프로그램

- **Fêtes Consulaires**: 2026년 9월 21~27일. 체류기간에는 9/21~25 ‘Consulats en fête’ 프로그램 후보가 겹친다. 주행사 마을은 9/26~27로 출발 후이므로 억지로 일정을 바꾸지 않는다.
- **Station Mue 프로그램**: 9월 26일까지 행사 후보. Confluence 선택일에만 확인.
- **공연 후보**: 체류기간 중 연극·오케스트라 일정이 있으나, Annecy와 도시산책을 우선하고 저녁 체력이 남을 때만 선택한다.
- **Musée sentimental 전시**: Musée des Beaux-Arts에서 9월 중 진행 후보. 우천 대체안으로만 본다.

---

## 23. 교통·안전 상세

### 23.1 대중교통 이용

- TCL 메트로·트램·버스는 관광에 충분하다.
- 혼잡한 메트로에서는 가방을 앞으로 돌리고 지퍼를 닫는다.
- 차량문 옆에서 휴대전화를 들고 있지 않는다.
- 늦은 밤 불편함을 느끼면 버스 운전기사에게 정류장 사이 하차 가능 여부를 문의할 수 있다.

### 23.2 역 이용

- Part-Dieu에는 경찰 관련 시설과 경비인력이 있으나, 규모가 큰 역이므로 혼잡 그 자체가 위험요소다.
- 플랫폼 전광판을 보느라 캐리어를 뒤에 두지 않는다.
- 카페의 의자 뒤에 가방을 걸지 않는다.
- Annecy 귀환 후 20시 이후에는 역에서 숙소로 바로 이동한다.

### 23.3 차량

리옹 시내에서는 차량을 보유하지 않는 것이 원칙이다. 불가피하게 차가 있으면 지하주차장을 사용하고, 트렁크를 포함해 어떤 물건도 외부에서 보이게 두지 않는다.

### 23.4 응급·지원

- 경찰: 17
- 유럽 공통 긴급번호: 112
- 구급: 15
- 소방: 18
- 야간에 불안하면 ‘Ask for Angela’ 참여 매장을 찾는 방법을 숙지한다.

---

## 24. 우천·지연·피로 대체안

| 상황 | 기본 대체안 |
|---|---|
| 9/22 비 | Fourvière는 푸니쿨라 왕복, Vieux Lyon 축소, Musée Gadagne 또는 Beaux-Arts 1곳 |
| 9/23 비 | Croix-Rousse 시장 짧게, Halles 체류 연장, Beaux-Arts 2시간 |
| Annecy 비 | Vieille Ville·Palais de l’Île·긴 점심, 크루즈 운항 시만 탑승 |
| Annecy 철도장애 | Lyon Confluence·Navigône·Parc 생활일 |
| 강한 피로 | Croix-Rousse와 Halles만 진행, Parc 삭제 |
| 소매치기·분실 | 즉시 카드 정지, 17 신고, 가까운 경찰서에서 신고서 작성 |
| 야간 체감불안 | 큰길·메트로 고집하지 않고 택시로 숙소 복귀 |

---

## 25. 예약카드

### 카드 A — Lyon 숙소

- 날짜: 9/21~9/25, 4박
- 1순위권: Ainay–Ampère
- 2순위권: Bellecour 남부 또는 Brotteaux–Masséna
- 필수: 엘리베이터, 냉방, 야간출입, 금고, 조용한 객실
- 총액: 관광세·조식·취소가능요금 포함

### 카드 B — Avignon TGV→Lyon

- 9/21 오전
- 렌터카 반납 완료 후 최소 60~90분 여유
- 직행 TGV 우선
- 큰 짐 좌석 가까운 선반 확보

### 카드 C — Annecy 왕복

- 9/24
- Lyon Part-Dieu 08시 전후 출발, 10시 전후 도착 목표
- 18~19시대 귀환편 목표
- 직행 우선, 변경·환불조건 확인

### 카드 D — Annecy 크루즈

- 9/24 14:30 전후 1시간 코스
- 날씨·운항·탑승장 재확인
- 예약 시 취소조건 확인

### 카드 E — 대표 저녁

- Café Comptoir Abel: 9/21
- Daniel et Denise: 9/22
- 늦은 귀가는 택시 가능하도록 식당 위치와 숙소 관계 확인

---

## 26. 출발 전 확인목록

- [ ] “프랑스 범죄율 1위”라는 민간순위 대신 공식 안전수칙을 기준으로 판단
- [ ] 숙소에서 메트로역까지 야간 보행경로를 지도 거리뷰로 확인
- [ ] Avignon 차량반납 시간과 Lyon TGV 시간의 여유 확인
- [ ] Lyon–Annecy 9/24 실제 직행시간과 공사 확인
- [ ] Annecy 크루즈 9월 운항표와 기상 확인
- [ ] 가방은 지퍼형 크로스백, 여권·카드는 분산보관
- [ ] 캐리어용 작은 잠금장치 또는 스트랩 준비
- [ ] Lyon 식당 2곳 이상 예약하지 않기
- [ ] Jason 러닝화 준비
- [ ] Julia 수영을 넣는 경우에만 수영장 시간표·수영모 규정 확인
- [ ] 9/25 Paris TGV 출발 90분 전 Part-Dieu 도착계획 세우기

---



---

## 편집자 큐레이션 — 리옹의 도시층과 안시의 호수를 더 깊게 보기

### 핵심 방문지 소개 보강

#### Fourvière — 전망대에서 지형을 먼저 읽는다

리옹은 Rhône과 Saône, 언덕과 반도 때문에 평면지도만으로 이해하기 어렵다. Fourvière에서 Vieux Lyon, Presqu’île, Part-Dieu 방향을 차례로 찾은 뒤 하산하면 이후 동선이 선명해진다.

- **꼭 해볼 것:** 두 강 구분, Bellecour 위치 찾기, 로마극장 방향 확인
- **적정 체류:** Basilica·전망 포함 60~90분

#### Vieux Lyon·traboules — ‘비밀통로 찾기’가 아니라 도시의 관통구조

traboule은 관광객용 미로가 아니라 길고 좁은 필지에서 거리와 Saône 강을 연결한 통로다. 공개표시가 있는 곳만 조용히 이용하고, 주거공간의 문을 억지로 열지 않는다.

- **관찰 포인트:** 안뜰 계단, 우물, 통로의 방향, 상층 갤러리
- **적정 체류:** 90분

#### Croix-Rousse — 전망보다 일하고 저항한 언덕

비단직조공 canut의 높은 천장 주택, 가파른 계단, 벽화와 시장을 함께 봐야 한다. 단순히 ‘힙한 동네’로 소비하지 않고 노동도시의 흔적을 읽는다.

#### Halles Paul Bocuse — 한 끼를 먹는 시장보다 리옹 식재료의 쇼케이스

샤퀴테리·치즈·quenelle·디저트를 모두 사지 않는다. 2인 기준 **전채 1 + quenelle 또는 따뜻한 요리 1 + 치즈 또는 디저트 1**로 작은 시식여행을 만든다.

#### Parc de la Tête d’Or — 장기여행의 회복을 위한 실제 생활공간

정원목록을 채우기보다 호수와 큰 산책로를 이용한다. Jason은 5~8km 러닝, Julia는 산책·카페·식물원 선택으로 분리했다가 합류할 수 있다.

#### Annecy — 운하사진보다 도시와 호수의 접점을 본다

Palais de l’Île 주변은 혼잡하므로 사진을 찍고 바로 호숫가로 이동한다. Jardins de l’Europe–Pont des Amours–Pâquier를 걸으며 구시가지의 좁은 수로가 넓은 알프스 호수로 열리는 전환을 느낀다.

- **꼭 해볼 것:** Thiou 운하, 구시가지 아케이드, 호숫가 산책, 날씨 좋으면 1시간 크루즈
- **적정 체류:** 6~7시간

### 리옹에서 꼭 체험할 것

1. **푸니쿨라로 올라 걸어서 내려오기:** 지형을 몸으로 이해한다.
2. **공개 traboule 2~3개만 조용히 통과:** 개수보다 구조 이해.
3. **Halles에서 작은 시식점심:** 과식하지 않는다.
4. **두 강 비교산책:** Saône의 역사적 경관과 Rhône의 현대적 보행축 비교.
5. **bouchon 한 번만 제대로:** 나머지 저녁은 가벼운 비스트로·숙소식.

### 식당 소개 보강

#### Café Comptoir Abel — 역사적 분위기와 대표요리를 함께

Ainay 숙소권과 연결하기 좋다. quenelle 또는 poulet aux morilles 중 하나를 중심으로 하고, 전채를 공유해 과식을 피한다. 첫날의 ‘리옹 입문식’에 적합하다.

#### Daniel et Denise — 정통 bouchon을 가장 체계적으로 경험

pâté en croûte, quenelle, praline 디저트를 모두 주문하면 무겁다. 전채 1개 공유 + 메인 2개 + 디저트 1개 공유가 균형이 좋다. 예약시간을 엄수한다.

#### Halles Paul Bocuse — 식당 대신 점포를 조합하는 점심

Giraudet의 quenelle, Sibilia의 charcuterie, Mère Richard의 치즈 가운데 2곳만 고른다. 구매·시식·앉을 자리를 먼저 확인한다.

#### L’Artichaut — 전통음식 뒤의 가벼운 현대 비스트로

전날 bouchon을 먹었다면 채소·생선 중심의 메뉴를 선택하는 완충저녁으로 적합하다.

#### Annecy Savoy 점심 — fondue는 날씨와 컨디션을 보고

치즈 fondue·tartiflette는 호숫가 산책 뒤 만족감이 크지만 무겁다. 따뜻한 날에는 perch·salad·diots 중 가벼운 구성을 선택하고, fondue는 두 사람이 공유 가능한지 확인한다.

### 치안과 분위기를 함께 고려한 현장 규칙

- Part-Dieu에서는 열차 출발 직전까지 휴대전화와 가방을 노출하지 않는다.
- Vieux Lyon의 혼잡구간에서는 가방을 의자 뒤에 걸지 않는다.
- 늦은 공연이 없으므로 밤 22시 이후 장거리 도보를 관광목적으로 만들지 않는다.
- Ainay·Brotteaux의 밝은 대로와 단순한 귀가동선을 숙소선택에서 우선한다.

### 현장 선택 규칙

- **안시 날씨가 좋으면:** 크루즈 추가
- **비가 오면:** 구시가지 + Château/Museum 선택, 호숫가 축소
- **리옹 도시일이 피곤하면:** Croix-Rousse 또는 Parc 중 하나만
- **대표식:** quenelle 1회, praline 디저트 1회
- **가장 중요한 장면:** Fourvière의 도시전망과 Annecy 호수의 열린 수평선


## 27. 공식자료

- 프랑스 내무부·SSMSI 2025 시·구 단위 범죄지표: https://www.interieur.gouv.fr/actualites/communiques-de-presse/geographie-de-delinquance-a-lechelle-communale-en-2025
- 프랑스 내무부·SSMSI 범죄통계 데이터: https://www.data.gouv.fr/datasets/bases-statistiques-communale-departementale-et-regionale-de-la-delinquance-enregistree-par-la-police-et-la-gendarmerie-nationales
- Rhône 주정부 Part-Dieu 안전강화 작전(2026년 6월): https://www.rhone.gouv.fr/Actualites/Espace-Presse/Les-communiques-de-presse-2026/JUIN-2026/Operation-de-securisation-sur-le-secteur-de-la-Part-Dieu
- Lyon Tourist Office 안전안내: https://en.visiterlyon.com/practical-lyon/safety-first-in-lyon
- Lyon Tourist Office: https://en.visiterlyon.com/
- TCL 요금·교통: https://www.tcl.fr/ 및 https://carte-interactive.tcl.fr/fares
- SNCF Avignon TGV–Lyon: https://www.sncf-connect.com/en-en/train/timetables/avignon-tgv/lyon
- SNCF Lyon–Annecy: https://www.sncf-connect.com/en-en/train/timetables/lyon/annecy
- Lake Annecy Tourist Office 구시가지: https://en.lac-annecy.com/must-sees/the-old-town-of-annecy/
- Lake Annecy 크루즈: https://en.lac-annecy.com/service/compagnie-des-bateaux-du-lac-dannecy-annecy/
- Ville de Lyon 행사·시장·체육시설 공식자료
- 각 숙소와 식당의 공식사이트 또는 Lyon Tourist Office 등재정보

> **최종 실무 메모**: 치안은 과장된 도시랭킹보다 시간·장소·행동별 위험으로 관리한다. 리옹을 피할 필요는 없으며, **Ainay 또는 6구의 안정적인 숙소, 수하물일 택시, 관광지와 대중교통의 소매치기 방지**가 이번 여행에 필요한 현실적 대응이다.
