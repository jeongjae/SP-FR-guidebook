...
### Shortened — Aix 챕터 내부 중복 (G003)

- `07_Aix_en_Provence_v2.0.md`: `### 14.1 엑상에서 먹어야 할 것` 표 제거.
  신설 `## 먹어야 할 것` 절(같은 8개 항목 + 설명)의 완전 부분집합이었다
  (유사도 93.5%). 식당 후보·날짜별 배치(고유 정보)는 유지, 정본 참조 한 줄로 대체.
- 앵커 참조 확인: 자체 페이지·검색 인덱스뿐 — 외부 딥링크 없음.
